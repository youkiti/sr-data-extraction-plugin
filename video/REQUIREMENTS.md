...
> **シーン番号 `00` は予約番号**: `scenes/examples/`（スモークテスト等）専用とし、実チャプターの
> 番号（01〜14）には使わない。`assemble.mjs` は `00-` 始まりのシーンキーを最終動画の対象から
> 自動的に除外するため、過去のスモーク収録が `video/build/` に残っていても本編に混入しない。
