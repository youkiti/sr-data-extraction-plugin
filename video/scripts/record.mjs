...
//   node video/scripts/record.mjs 00-smoke 03-documents  # 指定シーンのみ収録
...
async function recordScene(sceneFile, extensionDir) {
    ...
    const profileDir = mkdtempSync(path.join(os.tmpdir(), `sr-dep-video-profile-${sceneKey}-`));
    const videoDir = mkdtempSync(path.join(os.tmpdir(), `sr-dep-video-rec-${sceneKey}-`));
