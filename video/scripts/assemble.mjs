 ...
 * `00-` 始まり（examples/ のスモークテスト等）は本編の対象外として除外し、
 * その旨を console.log で明示する（黙って落とさない）。
 */
...
function buildDescription(chaptersText, version) {
    return [
        'SR Data Extraction Plugin の操作解説動画です。',
        'システマティックレビュー／スコーピングレビューの「データ抽出工程」を支援する',
        'Chrome 拡張機能の使い方を、準備からプロジェクト作成、文献の取り込み、',
        'プロトコル登録、表のデザイン（抽出項目）、パイロット抽出、一括抽出、検証、',
        'ダッシュボード、独立二重レビューと裁定、エクスポート、設定まで、ひととおり解説します。',
        '',
        ...
        '',
        `SR Data Extraction Plugin v${version}`,
    ].join('\n') + '\n';
}
...
async function main() {
    ...
    if (built.length === 0) {
        // スモーク（00-）しか収録されていない場合もここに来る（listBuiltScenes が除外済み）。
        // スモークだけの状態で最終動画を作る意味は無いため、実チャプター収録を促して止める。
        throw new Error(
            `video/build/scenes/ に本編シーンがありません。先に実チャプターを収録してください` +
            `（00- 始まりのスモーク/サンプル用シーンは対象外です）: ${BUILD_SCENES_DIR}`,
        );
    }
    mkdirSync(BUILD_DIR, { recursive: true });
    const tmpDir = mkdtempSync(path.join(os.tmpdir(), 'sr-dep-video-assemble-'));
