...
//
// tiab-review-plugin/video からの移植時の適応点（PR1）:
...
/**
 * 収録対象の拡張機能ビルドディレクトリを解決する（PR1 での本リポジトリ固有の適応点）。
 *
 * tiab-review-plugin は `dist-demo/`（デモ専用ビルド）に固定していたが、本拡張は
 * まだデモビルド層を持たない。そのため以下の優先順位で解決する。
 *   1. 環境変数 EXT_DIST_DIR （明示指定。存在しなければエラー）
 *   2. `<repo>/dist-demo`（存在すれば。デモビルドが後続 PR で追加された場合に自動的に
 *      そちらを優先させるための布石）
