...
//
// 見た目: 白地に緑の輪郭を持つ矢印カーソル（本拡張の公開ページのトンマナに合わせた
// 配色 #1b5e4a / #2f8f6d）+ クリック時のリップル（波紋）アニメーション。
...
function installCursorScript() {
    ...
    const CURSOR_COLOR = '#1b5e4a';
    const RIPPLE_COLOR = '#2f8f6d';
    ...
    const setup = () => {
        ...
        // 矢印カーソル（SVG）。白地+緑の輪郭で、明暗どちらの背景でも視認できるようにする。
        cursor.innerHTML =
