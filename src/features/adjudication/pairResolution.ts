// 対象 annotator ペアの解決（docs/design-independent-dual-review.md §6.1・§13）。
// StudyData / ResultsData / Decisions のいずれかに human 系 annotator（human_with_ai /
// human_independent。'ai' と 'consensus' は除外）の行を持つ email を study ごとに列挙する。
// ちょうど 2 名は自動でペア確定（ready）、3 名以上は裁定者が一覧で 2 名を選ぶ（selectable。
// issue #63 の 3 人以上対応）、1 名以下は検証完了待ち（waiting）として画面内の案内に委ねる
 ...
 * 1 名以下 → waiting（両者の検証完了待ち）。3 名以上 → selectable（裁定者が 2 名を選ぶ）
 */
export function resolveAnnotatorPair(input: ResolveAnnotatorPairInput): AnnotatorPairResolution {
  ...
  if (sorted.length >= 3) {
    return { kind: 'selectable', annotators: sorted };
  }
