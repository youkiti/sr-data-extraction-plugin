...
//
// bbox 列（bbox_page/bbox_ymin/bbox_xmin/bbox_ymax/bbox_xmax。§7.4 PR3）は既存プロジェクトに
// 存在しないことがあるため、読み書きの両方で後方互換を取る:
// - 読み出し（readEvidenceRows）: 先頭 12 列（旧ヘッダ）は厳格一致、13 列目以降は
//   「存在すれば」名前一致を要求し「欠けていれば」旧プロジェクトとして許容する
// - 書き込み（ensureEvidenceBboxColumns）: 旧 12 列ヘッダのプロジェクトへは実行前に
//   ヘッダ行を 17 列のフルヘッダへ拡張する（呼び出しは extractionService.ts の責務）
 ...
 *
 * ヘッダ検証: 先頭 12 列（旧ヘッダ）は厳格一致。13 列目以降（bbox 5 列）は
...
export async function readEvidenceRows(
  ...
  if (header.length > LEGACY_COLUMN_COUNT) {
    SHEET_HEADERS.Evidence.slice(LEGACY_COLUMN_COUNT).forEach((name, i) => {
      const idx = LEGACY_COLUMN_COUNT + i;
      if (cellAt(header, idx) !== name) {
        throw new Error(
          `Evidence のヘッダ ${idx + 1} 列目が "${name}" ではありません（実際: "${cellAt(header, idx)}"）`,
        );
      }
    });
  }
