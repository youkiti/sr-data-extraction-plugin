...
export interface AiAnnotationRows {
  /** entity_level = study の項目 → StudyData の ai 行（1 document 1 行、wide） */
 ...
 * - 同一セル（document × field × entity_key）に複数の Evidence がある場合は後勝ち
...
export function buildAiAnnotationRows(
  ...
  // StudyData: document_id → values（field_name → セル値）
  const studyValues = new Map<string, Record<string, string | null>>();
  // ResultsData: 更新キー（document × entity_key × field）→ 行。後勝ちで上書き。
  ...
  for (const item of evidence) {
    ...
    if (field.entityLevel === 'study') {
      let values = studyValues.get(item.documentId);
      if (values === undefined) {
        values = {};
        studyValues.set(item.documentId, values);
      }
      values[field.fieldName] = toCellValue(item);
    } else {
      resultsByKey.set(JSON.stringify([item.documentId, item.entityKey, item.fieldId]), {
  ...
  const studyRows: StudyDataRow[] = [...studyValues.entries()].map(([documentId, values]) => ({
    documentId,
    annotator: AI_ANNOTATOR,
    annotatorType: 'ai',
    schemaVersion: params.schemaVersion,
