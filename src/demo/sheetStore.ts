// デモモード用インメモリ「スプレッドシート」ストア（tiab-review-plugin/src/demo/sheet-store.ts
// の方式を移植）。
//
// Google Sheets API のレスポンス形状を模すため、シート名 → 行データ（ヘッダー行含む、
// 各行は string[]）の形で状態を保持し、A1 形式のレンジ指定（"Sheet!A1:Z1" 等）を
// 解釈して読み書きする。fetchMock.ts から呼び出される、副作用を持つ唯一のモジュール。
// タブ構成・ヘッダ自体はここでは持たない（domain/sheetsSchema.ts の SHEET_TABS /
// SHEET_HEADERS を単一の正典として seed.ts が読み込み、resetDemoStore へ渡す）。
//
// ページセッション中（= app.html を開いてから閉じる/再読込するまで）は状態を保持するため、
// 検証の判定保存 → 再読込でも内容が残る。app.html はハッシュルーティングの単一ページなので、
// 画面遷移をまたいでも本モジュールのモジュールスコープ変数がそのまま生き続ける。

/** ストア全体の状態。resetDemoStore() でのみ丸ごと差し替える */
interface DemoSheetState {
  title: string;
  /** シート名 → 行データ（0 番目の要素がスプレッドシートの 1 行目 = ヘッダー行想定） */
  sheets: Map<string, string[][]>;
  /** シート名 → addSheet で払い出した sheetId（sheets.properties 一覧・addSheet 応答用） */
 ...
 * デモではヘッダー未整備時の「無ければ作る」再試行フロー（ArmStructures / Reviewers 等）を
 * 壊さないための安全策）。
 */
...
/** 指定レンジの値を取得する。シートが存在しない場合は null（呼び出し側で 404 相当に変換する） */
...
export function writeRange(range: string, values: string[][]): boolean {
  ...
  values.forEach((rowValues, i) => {
    ...
    const row = rows[targetRowIndex] as string[];
 ...
 * 戻り値の firstRowIndex / lastRowIndex は追記した行のシート行番号（1 始まり）。
 * シートが存在しない場合は null。
 */
