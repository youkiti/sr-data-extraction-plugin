...
次のステップ: S3 画面の UI（`lib/google/picker.ts` + documentsView: Picker 起動 → `importDocuments` 呼び出し・進捗行・text_status バッジ・study_label インライン編集）→ 以降 protocol → schema → … の画面実装（test-strategy.md §3。画面完成ごとにルート別 E2E + axe）と app シェルの Sheets 読込（進捗カウント）。テストは jest（カバレッジ 100% 強制）+ Playwright smoke + CI-1 が稼働済みのため、作業原則 7・8 は**有効**。
