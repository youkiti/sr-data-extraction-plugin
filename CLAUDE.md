...
次のステップ: フェーズ 1 の画面実装の続き（documents → protocol → schema → …。test-strategy.md §3）と app シェルの Sheets 読込（進捗カウント）。documents（S3）実装時に extracted_texts の保存形式を確定し、`extractionService` の `loadDocumentPages` を実装して注入する（S6 / S7 画面から `runExtraction` を呼ぶ結線もその後）。テストは jest（カバレッジ 100% 強制）+ Playwright smoke + CI-1 が稼働済みのため、作業原則 7・8 は**有効**。
