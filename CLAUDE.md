...
次のステップ: S7 一括抽出 → S9 ダッシュボード（**このとき `#/verify` の `?entity=` ディープリンクを一緒に実装する**。ui-flow.md §3 の注記参照）→ S10 エクスポートの画面実装（test-strategy.md §3。画面完成ごとにルート別 E2E + axe）と app シェルの Sheets 読込（進捗カウント）。あわせて実機での Picker 動作確認（hosted/picker.html はデプロイ済み。hosted/README.md）を行う。テストは jest（カバレッジ 100% 強制）+ Playwright smoke + CI-1 が稼働済みのため、作業原則 7・8 は**有効**。
