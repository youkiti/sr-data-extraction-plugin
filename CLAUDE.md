...
| ドキュメント | 内容 |
|---|---|
| [docs/requirements.md](docs/requirements.md) | 要件定義書 v0.3。データ設計（Sheets 9 タブ）・機能要件（S1〜S11）・quote アンカリング方式・未決定事項 Q1〜Q9（レビュー済み。Q4 の二重抽出は `Extractions` へのレビュアー軸追加が次版課題） |
...
- **追記型・上書き禁止**: 人間判定（`Extractions` の human 列）以外のタブは追記のみ
...
- **automation bias 対策**: 既定ステータス `unreviewed`、accept にも 1 操作必須、unreviewed 残存時のエクスポート警告
- **著作権**: 取り込み時の利用許諾確認チェック必須 + 記録列。PDF の外部送信は LLM API のみ
