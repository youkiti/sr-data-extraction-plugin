...
次のステップ: protocol（S4。sr-query-builder の protocol 画面 UI 移植）→ schema → … の画面実装（test-strategy.md §3。画面完成ごとにルート別 E2E + axe）と app シェルの Sheets 読込（進捗カウント）。あわせて hosted/picker.html のデプロイ（GitHub Pages。API キーは設定済み。hosted/README.md）と実機での Picker 動作確認を行う。テストは jest（カバレッジ 100% 強制）+ Playwright smoke + CI-1 が稼働済みのため、作業原則 7・8 は**有効**。
