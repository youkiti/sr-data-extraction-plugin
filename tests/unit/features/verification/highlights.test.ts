...
describe('buildDocumentHighlights', () => {
  ...
  test('exact は全ページの全出現を列挙し、ai_page に最も近い出現を既定にする', () => {
    const [highlight] = buildDocumentHighlights([makeEvidence({ page: 2 })], pages);
  ...
  test('ai_page が null なら先頭の出現を既定にする', () => {
    const [highlight] = buildDocumentHighlights([makeEvidence({ page: null })], pages);
    expect(highlight?.selectedIndex).toBe(0);
  });

  test('ai_page が 1 ページ目なら先頭の出現が最近傍として残る', () => {
    const [highlight] = buildDocumentHighlights([makeEvidence({ page: 1 })], pages);
    expect(highlight?.selectedIndex).toBe(0);
  });
  ...
  test('正規化で空になる quote（空白のみ）は含めない', () => {
    expect(buildDocumentHighlights([makeEvidence({ quote: '   ' })], pages)).toEqual([]);
  });

  test('どのページにも見つからない quote は含めない', () => {
    expect(
      buildDocumentHighlights([makeEvidence({ quote: 'not in the document at all' })], pages),
    ).toEqual([]);
  });
  ...
  test('fuzzy でページ番号が見つからない場合は含めない', () => {
    expect(
      buildDocumentHighlights([makeEvidence({ page: 9, anchorStatus: 'fuzzy' })], pages),
    ).toEqual([]);
  });
