...
describe('describeBatchFailure', () => {
  test('section なしは reason（detail）、section ありは先頭に section を付ける', () => {
    expect(
      describeBatchFailure({ studyId: 's1', section: null, reason: 'api_error', detail: 'boom' }),
    ).toBe('api_error（boom）');
    expect(
      describeBatchFailure({
        studyId: 's1',
...
describe('createStudyProgressTracker', () => {
  test('初期状態は全 study が待機中（指定順）+ バッチ数 0/総数', () => {
    ...
    expect(tracker.rows()).toEqual([
      { studyId: 's1', status: 'queued', completedBatches: 0, totalBatches: 1, detail: null },
      { studyId: 's2', status: 'queued', completedBatches: 0, totalBatches: 1, detail: null },
    ]);
  });

  test('計画に 1 バッチも現れない study は最初から failed（総バッチ数 0）', () => {
  ...
  test('計画外 study のバッチ・進捗は無視する（契約上は起こらない）', () => {
    ...
    expect(tracker.rows()).toEqual([
      { studyId: 's1', status: 'done', completedBatches: 1, totalBatches: 1, detail: null },
    ]);
  });
});
