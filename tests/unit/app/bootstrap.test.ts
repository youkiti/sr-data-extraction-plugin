...
describe('createChromeAppDeps', () => {
  test('loadApiKey はプロバイダ別に secretsStore のキーを引く', async () => {
    ...
    chromeMock.storage.local.data['secrets.azureOpenAiApiKey'] = 'azure-key';
    ...
    // issue #127 PR3: 'azure_openai' 未対応のまま Gemini キーへ黙ってフォールバックする回帰の防止
    // （PR2 で Anthropic に対して発生した実バグと同型）
    await expect(deps.loadApiKey('azure_openai')).resolves.toBe('azure-key');
...
describe('bootstrapApp: #/pilot', () => {
  ...
  test('#/pilot のレイアウトモードトグルが onChangeLayoutMode（setPilotLayoutMode）に配線されている', async () => {
    ...
    // 未設定時の開始比率は実測（leftPane 幅 / panesEl 幅）のため、leftPane 側も stub する（issue #193 F1）
    ...
    const leftPane = document.querySelector('.verify__pane--pdf') as HTMLElement;
    ...
    leftPane.getBoundingClientRect = jest.fn(
      () =>
        ({ width: 700, height: 0, top: 0, left: 0, right: 0, bottom: 0, x: 0, y: 0, toJSON: () => ({}) }) as DOMRect,
    );
...
describe('bootstrapApp: 独立二重レビュー機能', () => {
  ...
  test('reviewer_with_ai ロール + 不足分あり: 「読めないファイルをスキップして続行」ボタンが onSkipMissingFiles へ配線されている（issue #141）', async () => {
    const stub = createWindowStub({
      currentProject: PROJECT,
      home: COUNTS_LOADED,
      counts: { evidenceRows: 1 } as AppState['counts'],
      role: {
        ...
        folderAccessChecking: false,
        folderAccessError: null,
        folderAccessMissingCount: 2,
      },
    });
    const { deps } = createFakeDeps([]);
    await bootstrapApp(asWindow(stub), deps);

    // fake sheets は空応答のため Documents 読み出しでヘッダ検証エラーになる
    // = skipMissingFileAccess まで配線されている証明（grantFolderAccess と同じ手法）
    (document.getElementById('home-skip-missing-files') as HTMLButtonElement).click();
    await flush();
    expect(toastTexts()).toContain(
      'ファイルへのアクセスを確認できませんでした: Documents タブにヘッダ行がありません（プロジェクト初期化が不完全です）',
    );
...
describe('bootstrapApp: ロール解決の初期化順序とフェイルクローズ（盲検ガード）', () => {
  ...
  function createRoleDeps(options: {
    ...
    /** 先頭 n 回の fetch を 404 にする（drive.file アクセス拒否 → SheetsAccessDeniedError 経路） */
    accessDeniedBeforeSuccess?: number;
    gate?: Promise<void>;
    /** 指定時のみ Documents タブへヘッダ行付きの応答を返す（未指定は従来どおり values: [] で
     * 「ヘッダ行がありません」エラーになる。issue #141: 起動時の差分検知 checkMissingFileAccess の
     * 実弾テストで Documents 読み出しを成功させるために使う） */
    documentsRows?: string[][];
  }): { deps: AppDeps; fetchMock: jest.Mock } {
    let failures = options.failuresBeforeSuccess ?? 0;
    let denials = options.accessDeniedBeforeSuccess ?? 0;
    const fetchMock = jest.fn(async (input: RequestInfo | URL) => {
      ...
      } else if (options.documentsRows !== undefined && url.includes('/values/Documents')) {
        json = { values: [[...SHEET_HEADERS.Documents], ...options.documentsRows] };
  ...
  test('ロール解決失敗はフェイルクローズ（エラー画面 + ナビ非表示 + ローダ不発火）。再試行で復帰する', async () => {
    ...
    expect(document.querySelectorAll('#app-nav a')).toHaveLength(0);
    expect(calledBatchGet(fetchMock)).toBe(false);
    // アクセス拒否以外のエラーでは許可導線を出さない（issue #131）
    expect(document.getElementById('app-role-grant')).toBeNull();
  ...
  test('アクセス拒否ならロールエラーに「Google で許可する」を表示し、許可 → 再解決で復帰する（issue #131）', async () => {
    chromeMock.storage.local.data[CURRENT_PROJECT_STORAGE_KEY] = PROJECT;
    const stub = createWindowStub();
    const { deps } = createRoleDeps({
      email: 'owner@example.com',
      accessDeniedBeforeSuccess: 1,
    });
    // Picker fake: タブが開いたらフラグメントの nonce を拾い、対象シートの picked を返す
    let listener:
      | ((
          message: unknown,
          sender: { tabId: number | null; url: string | null },
    ...
    deps.picker = {
      ...
      extensionId: 'ext',
      pickerPageUrl: 'https://example.com/picker.html',
      createTab: jest.fn(async (url: string) => {
        const nonce = new URLSearchParams(url.split('#')[1] ?? '').get('nonce');
        // リスナー登録（createTab の await 継続）より後に届かせる
        setTimeout(() => {
          listener?.(
            {
              ...
              kind: 'picked',
              nonce,
              files: [{ id: 'sheet-1', name: 'テスト SR' }],
            },
            { tabId: 9, url: 'https://example.com/picker.html#x' },
            jest.fn(),
          );
        }, 0);
        return 9;
      }),
      removeTab: jest.fn(async () => undefined),
      addExternalMessageListener: (l) => {
        listener = l;
        return () => undefined;
      },
      addTabRemovedListener: jest.fn(() => () => undefined),
    ...
    const grant = document.getElementById('app-role-grant') as HTMLButtonElement;
    expect(grant).not.toBeNull();
    expect(document.getElementById('app-role-error')?.textContent).toContain(
      '権限がまだありません',
    );
    // 再試行ボタンも従来どおり残す
    expect(document.getElementById('app-role-retry')).not.toBeNull();

    grant.click();
    // Picker タブが開いている間は二重起動を防ぐ
    expect(grant.disabled).toBe(true);
    await flush();
    await flush();
    expect(document.getElementById('app-role-error')).toBeNull();
  ...
  test('許可導線を Picker キャンセルで閉じたらエラー画面を維持し、ボタンは押し直せる（issue #131）', async () => {
    chromeMock.storage.local.data[CURRENT_PROJECT_STORAGE_KEY] = PROJECT;
    const stub = createWindowStub();
    const { deps, fetchMock } = createRoleDeps({
      email: 'owner@example.com',
    ...
    await bootstrapApp(asWindow(stub), deps);
    await flush();
    (document.getElementById('app-role-grant') as HTMLButtonElement).click();
    await flush();
    await flush();
    // キャンセル → ブロック維持 + 再描画で新しいボタンは enabled + ルートローダは発火しない
    expect(document.getElementById('app-role-error')).not.toBeNull();
    expect((document.getElementById('app-role-grant') as HTMLButtonElement).disabled).toBe(false);
    expect(calledBatchGet(fetchMock)).toBe(false);
  });
  ...
  test('起動時の差分検知（checkMissingFileAccess）が loadRole 完了後に走る（issue #141）', async () => {
    chromeMock.storage.local.data[CURRENT_PROJECT_STORAGE_KEY] = PROJECT;
    // 付与済み（boolean ゲート）+ 付与済みレコードが既にある reviewer を模す
    chromeMock.storage.local.data['sr-data-extraction:folder-access-granted:sheet-1:reviewer@example.com'] =
      true;
    chromeMock.storage.local.data['sr-data-extraction:file-access-record:sheet-1:reviewer@example.com'] = {
      granted: [],
      skipped: [],
    };
    const stub = createWindowStub();
    const { deps } = createRoleDeps({
      email: 'reviewer@example.com',
      reviewersRows: [['reviewer@example.com', 'reviewer', 'with_ai', 'owner@example.com', 't0']],
      documentsRows: [], // ヘッダ行のみ（Documents 0 件）→ candidates 0 件 → missingCount=0
    });
    const store = await bootstrapApp(asWindow(stub), deps);
    await flush();

    const state = store?.getState();
    expect(state?.role.role).toBe('reviewer_with_ai');
    expect(state?.role.folderAccessGranted).toBe(true);
