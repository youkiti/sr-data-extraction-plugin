...
describe('自動遷移・初期フォーカス・スクロール保持（UX 改善）', () => {
  ...
  test('右ペインと text ビューのラッパーはストア再描画をまたぐ位置復元の対象（issue #192）', async () => {
    const { panel } = await createPanel();
    expect(
      panel.root.querySelector('.verify__pane--form')?.hasAttribute('data-preserve-scroll'),
    ).toBe(true);
    expect(
      panel.root.querySelector('.verify__text-body')?.hasAttribute('data-preserve-scroll'),
    ).toBe(true);
    panel.dispose();
  });
...
describe('「AI で再特定」（relocate-quote。issue #94）', () => {
  /** 再特定した Evidence（ev-2 を supersede）。quote は PAGES に実在する文言にして、
  ...
  /**
   * pending のまま制御できる onRelocateQuote（実行中状態の検証用）。
   * 戻り値の resolve/reject は「呼ばれた時点の現在の resolver」へ委譲する薄いラッパにする
   * （オブジェクトリテラルへ resolve/reject を直接詰めると、pendingRelocate() 呼び出し時点＝
   * まだ onRelocateQuote が一度も呼ばれていない＝Promise executor 未実行の初期値〔no-op〕を
   * 捕まえてしまい、後から button.click() 経由で実行された real executor の resolver に
   * 差し替わらない）
   */
  function pendingRelocate(): {
    onRelocateQuote: jest.Mock<Promise<RelocateQuoteOutcome>, [Evidence]>;
    resolve: (outcome: RelocateQuoteOutcome) => void;
    reject: (err: unknown) => void;
  } {
    let currentResolve: (outcome: RelocateQuoteOutcome) => void = () => undefined;
    let currentReject: (err: unknown) => void = () => undefined;
    const onRelocateQuote = jest.fn<Promise<RelocateQuoteOutcome>, [Evidence]>(
      () =>
        new Promise<RelocateQuoteOutcome>((res, rej) => {
          currentResolve = res;
          currentReject = rej;
        }),
    );
    return {
      onRelocateQuote,
      resolve: (outcome) => currentResolve(outcome),
      reject: (err) => currentReject(err),
    };
  }

  test('options.onRelocateQuote 未注入では「AI で再特定」ボタンを出さない', async () => {
    const { panel } = await createPanel();
    expect(cellEl(panel.root, KEY_COUNTRY)?.querySelector('.verify__quote-relocate')).toBeNull();
    panel.dispose();
  });

  test('成功: 実行中表示 → Evidence 追記済み行でハイライト・quote 表示が差し替わりジャンプする', async () => {
    const { onRelocateQuote, resolve } = pendingRelocate();
    const { panel } = await createPanel({}, { onRelocateQuote });
    const button = cellEl(panel.root, KEY_COUNTRY)?.querySelector<HTMLButtonElement>(
      '.verify__quote-relocate',
    );
    expect(button).not.toBeNull();
    button?.click();
    expect(onRelocateQuote).toHaveBeenCalledWith(expect.objectContaining({ evidenceId: 'ev-2' }));
    // 実行中は disabled + 文言変化
    const runningButton = cellEl(panel.root, KEY_COUNTRY)?.querySelector<HTMLButtonElement>(
      '.verify__quote-relocate',
    );
    expect(runningButton?.textContent).toBe('AI で再特定中…');
    expect(runningButton?.disabled).toBe(true);

    resolve({ status: 'relocated', evidence: makeRelocatedEvidence() });
    await flush();

    // 再特定成功 → unanchored フォールバック UI は消え、通常のハイライト UI（ジャンプ / 切替）が出る
    const quoteBlock = cellEl(panel.root, KEY_COUNTRY)?.querySelector('.verify__quote');
    expect(quoteBlock?.querySelector('.verify__quote-unanchored')).toBeNull();
    expect(quoteBlock?.querySelector('.verify__quote-relocate')).toBeNull();
    expect(quoteBlock?.querySelector('.verify__quote-jump')).not.toBeNull();
    // 表示中文書の PDF は読込済みのため、rectHighlightsByDoc への差分反映（recomputeRectHighlights）
    // が効いて実際のハイライト要素が描画される
    const highlight = [...panel.root.querySelectorAll<HTMLButtonElement>('.pdf-viewer__hl')].find(
      (node) => node.getAttribute('aria-label') === '根拠: 国', // f-country の fieldLabel
    );
    expect(highlight).toBeDefined();
    panel.dispose();
  });

  test('not_found: 案内メッセージを出し、ボタンは再度有効になる（従来の本文内検索も残る）', async () => {
    const { onRelocateQuote, resolve } = pendingRelocate();
    const { panel } = await createPanel({}, { onRelocateQuote });
    cellEl(panel.root, KEY_COUNTRY)?.querySelector<HTMLButtonElement>('.verify__quote-relocate')?.click();
    resolve({ status: 'not_found' });
    await flush();
    const quoteBlock = cellEl(panel.root, KEY_COUNTRY)?.querySelector('.verify__quote');
    expect(quoteBlock?.querySelector('.verify__quote-relocate-not-found')?.textContent).toBe(
      'AI でも見つかりませんでした。本文内検索をお試しください',
    );
    const button = quoteBlock?.querySelector<HTMLButtonElement>('.verify__quote-relocate');
    expect(button?.textContent).toBe('AI で再特定');
    expect(button?.disabled).toBe(false);
    // 従来の「本文内を検索」フォールバックも引き続き使える
    expect(quoteBlock?.querySelector('.verify__quote-search')).not.toBeNull();
    panel.dispose();
  });

  test('onRelocateQuote が reject されても not_found と同様に扱う', async () => {
    const { onRelocateQuote, reject } = pendingRelocate();
    const { panel } = await createPanel({}, { onRelocateQuote });
    cellEl(panel.root, KEY_COUNTRY)?.querySelector<HTMLButtonElement>('.verify__quote-relocate')?.click();
    reject(new Error('network down'));
    await flush();
    expect(
      cellEl(panel.root, KEY_COUNTRY)?.querySelector('.verify__quote-relocate-not-found'),
    ).not.toBeNull();
    panel.dispose();
  });

  test('実行中の多重クリックは 1 回しか呼ばない', async () => {
    const { onRelocateQuote } = pendingRelocate();
    const { panel } = await createPanel({}, { onRelocateQuote });
    const button = cellEl(panel.root, KEY_COUNTRY)?.querySelector<HTMLButtonElement>(
      '.verify__quote-relocate',
    );
    button?.click();
    button?.click();
    expect(onRelocateQuote).toHaveBeenCalledTimes(1);
    panel.dispose();
  });

  test('未読込の文書（PDF 未ロード）が出所の再特定成功は rectHighlightsByDoc への反映をスキップする', async () => {
    // f-country の出所を doc-2（study 内の別文書）にし、doc-1 だけを初期表示・ロードする。
    // recomputeRectHighlights は textPagesByDoc に doc-2 のエントリが無いため何もせず return する
    const { onRelocateQuote, resolve } = pendingRelocate();
    const { panel } = await createPanel(
      {
        documents: [
          makeDocFixture({ document: makeDocumentRecord({ documentId: 'doc-1' }) }),
          makeDocFixture({
            document: makeDocumentRecord({ documentId: 'doc-2', filename: 'other.pdf' }),
            textPages: [buildPage(1, 'other document text')],
          }),
        ],
        evidence: [
          ...EVIDENCE,
        ].map((item) => (item.evidenceId === 'ev-2' ? { ...item, documentId: 'doc-2' } : item)),
      },
      { onRelocateQuote },
    );
    cellEl(panel.root, KEY_COUNTRY)?.querySelector<HTMLButtonElement>('.verify__quote-relocate')?.click();
    resolve({
      status: 'relocated',
      evidence: makeRelocatedEvidence({ documentId: 'doc-2' }),
    });
    await flush();
    // クラッシュせず、ボタンは通常のハイライト UI へ切り替わる（doc-2 は未読込のため実際の
    // ハイライト要素は無いが、cell.evidence 自体は正しく差し替わっている）
    expect(cellEl(panel.root, KEY_COUNTRY)?.querySelector('.verify__quote-relocate')).toBeNull();
    panel.dispose();
  });

  test('文書は読込済みだが quote が本文中に見つからない再特定結果は差分ハイライトを追加しない', async () => {
    // 通常は relocateQuoteService が事前にアンカリング検証済みの quote しか返さないため
    // 起こらない想定だが、extracted_texts と PDF テキスト層が取り込み後に乖離した場合の防御を確認する
    const { onRelocateQuote, resolve } = pendingRelocate();
    const { panel } = await createPanel({}, { onRelocateQuote });
    cellEl(panel.root, KEY_COUNTRY)?.querySelector<HTMLButtonElement>('.verify__quote-relocate')?.click();
    resolve({
      status: 'relocated',
      evidence: makeRelocatedEvidence({ quote: 'this text is not present in any page' }),
    });
    await flush();
    expect(cellEl(panel.root, KEY_COUNTRY)?.querySelector('.verify__quote-relocate')).toBeNull();
    panel.dispose();
  ...
  test('抽出テキストモード表示中の成功はテキストビューアの根拠スニペットを差し替える', async () => {
    const { onRelocateQuote, resolve } = pendingRelocate();
    const { panel } = await createPanel({}, { onRelocateQuote });
    // 抽出テキストモードへ切替
    const textModeButton = [...panel.root.querySelectorAll<HTMLButtonElement>('.verify__view-toggle-btn')].find(
      (btn) => btn.textContent === '抽出テキスト',
    );
    textModeButton?.click();
    cellEl(panel.root, KEY_COUNTRY)?.querySelector<HTMLButtonElement>('.verify__quote-relocate')?.click();
    resolve({ status: 'relocated', evidence: makeRelocatedEvidence() });
    await flush();
    expect(panel.root.querySelector('.text-viewer__body')?.textContent).toContain(
      'mortality was 12 percent',
    );
    panel.dispose();
  });
...
describe('createVerificationPanel: ペインサイズ調整（issue #193）', () => {
  ...
  function firePointer(el: HTMLElement, type: string, clientX = 0, clientY = 0): void {
    el.dispatchEvent(new MouseEvent(type, { clientX, clientY, bubbles: true }));
  }

  function widthSplitterEl(root: HTMLElement): HTMLElement {
    return root.querySelector<HTMLElement>('.verify__splitter--vertical')!;
  }
  function heightSplitterEl(root: HTMLElement): HTMLElement {
    return root.querySelector<HTMLElement>('.verify__splitter--horizontal')!;
  }
  ...
  test('スプリッタ 2 種の ARIA 属性（role=separator + aria-orientation + aria-label + tabindex）', async () => {
    ...
    const h = heightSplitterEl(panel.root);
    expect(w.getAttribute('role')).toBe('separator');
    expect(w.getAttribute('aria-orientation')).toBe('vertical');
    expect(w.getAttribute('aria-label')).toBe('PDF ペインと判定項目枠の左右比率を調整');
  ...
  test('未設定（既定）: カスタムプロパティは設定されず、aria-valuenow は既定比率・実用下限高さを示す', async () => {
    const { panel } = await createPanel();
    ...
    // formPane の getBoundingClientRect は既定で高さ 0（jsdom）→ 実用下限 240 を示す
    expect(heightSplitterEl(panel.root).getAttribute('aria-valuenow')).toBe('240');
    panel.dispose();
  });

  test('options.paneLayout を初期適用する（新規パネル生成時）', async () => {
    const onPaneLayoutChange = jest.fn();
    const { panel } = await createPanel(
      {},
  ...
  test('左右スプリッタ: ドラッグ中は DOM 直書きのみ・pointerup で 1 回だけ永続化コールバックを呼ぶ', async () => {
    ...
    expect(onPaneLayoutChange).toHaveBeenCalledWith(
      expect.objectContaining({ pdfPaneRatio: expectedRatio }),
    );
    panel.dispose();
  });
  ...
  test('左右スプリッタ: pointerdown なしの pointermove / pointerup / pointercancel は no-op', async () => {
    const onPaneLayoutChange = jest.fn();
    const { panel } = await createPanel({}, { onPaneLayoutChange });
    const splitter = widthSplitterEl(panel.root);
    stubRect(panesEl(panel.root), { width: 1200 });
  ...
  test('左右スプリッタ: ダブルクリックで既定比率へリセットする', async () => {
    const onPaneLayoutChange = jest.fn();
    const { panel } = await createPanel(
      {},
      { paneLayout: { formPaneHeight: null, pdfPaneRatio: 0.6 }, onPaneLayoutChange },
  ...
  test('左右スプリッタ: 矢印キー以外は無視し、クランプ不能ならキー操作も確定しない', async () => {
    const onPaneLayoutChange = jest.fn();
    const { panel } = await createPanel({}, { onPaneLayoutChange });
    const splitter = widthSplitterEl(panel.root);
    stubRect(panesEl(panel.root), { width: 1200 });
    ...
    // コンテナ幅を stub しない panel で押すとクランプ不能（no-op）
    const { panel: panel2 } = await createPanel({}, { onPaneLayoutChange });
    widthSplitterEl(panel2.root).dispatchEvent(
  ...
  test('高さスプリッタ: ドラッグでフォームペインの高さを変更し、pointerup で 1 回だけ確定する', async () => {
    ...
    const { panel } = await createPanel({}, { onPaneLayoutChange });
    const splitter = heightSplitterEl(panel.root);
    stubRect(formPaneEl(panel.root), { height: 500 }); // measured > 0 の分岐
    ...
    expect(onPaneLayoutChange).toHaveBeenCalledTimes(1);
    expect(onPaneLayoutChange).toHaveBeenCalledWith(
      expect.objectContaining({ formPaneHeight: 560 }),
    );
    panel.dispose();
  });

  test('高さスプリッタ: 未設定 + 高さ測定不能（既定 0）のときは実用下限からドラッグを開始する', async () => {
    const onPaneLayoutChange = jest.fn();
    const { panel } = await createPanel({}, { onPaneLayoutChange });
    const splitter = heightSplitterEl(panel.root);
    // formPane の getBoundingClientRect は既定で高さ 0（jsdom）のまま stub しない

    firePointer(splitter, 'pointerdown', 0, 200);
    firePointer(splitter, 'pointermove', 0, 220); // 実用下限 240 + 20px → 260px
    expect(formPaneEl(panel.root).style.getPropertyValue('--verify-form-height')).toBe('260px');
    panel.dispose();
  });

  test('高さスプリッタ: 既に設定済みの高さからドラッグを開始する', async () => {
    const onPaneLayoutChange = jest.fn();
    const { panel } = await createPanel(
      {},
    ...
    firePointer(splitter, 'pointerdown', 0, 200);
    firePointer(splitter, 'pointermove', 0, 220); // +20px → 520px
    expect(formPaneEl(panel.root).style.getPropertyValue('--verify-form-height')).toBe('520px');
    firePointer(splitter, 'pointerup', 0, 220);
    expect(onPaneLayoutChange).toHaveBeenCalledWith(
      expect.objectContaining({ formPaneHeight: 520 }),
    );
    panel.dispose();
  });

  test('高さスプリッタ: pointerdown なしの pointermove / pointerup は no-op', async () => {
    const onPaneLayoutChange = jest.fn();
    const { panel } = await createPanel({}, { onPaneLayoutChange });
    const splitter = heightSplitterEl(panel.root);
    firePointer(splitter, 'pointermove', 0, 260);
    firePointer(splitter, 'pointerup', 0, 260);
    firePointer(splitter, 'pointercancel', 0, 260);
    expect(onPaneLayoutChange).not.toHaveBeenCalled();
    panel.dispose();
  });
  ...
  test('高さスプリッタ: ダブルクリックで既定（未設定）へリセットする', async () => {
    ...
    const { panel } = await createPanel(
      {},
      { paneLayout: { formPaneHeight: 900, pdfPaneRatio: null }, onPaneLayoutChange },
    );
    const splitter = heightSplitterEl(panel.root);
    splitter.dispatchEvent(new MouseEvent('dblclick', { bubbles: true }));
    expect(onPaneLayoutChange).toHaveBeenCalledWith(
  ...
  test('高さスプリッタ: 矢印キー（↓ / ↑）で高さを増減し、都度確定する。矢印キー以外は無視する', async () => {
    const onPaneLayoutChange = jest.fn();
    const { panel } = await createPanel(
      {},
      { paneLayout: { formPaneHeight: 500, pdfPaneRatio: null }, onPaneLayoutChange },
    ...
    expect(onPaneLayoutChange).toHaveBeenCalledWith(expect.objectContaining({ formPaneHeight: 524 }));
    onPaneLayoutChange.mockClear();

    splitter.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowUp', bubbles: true }));
    expect(onPaneLayoutChange).toHaveBeenCalledWith(expect.objectContaining({ formPaneHeight: 500 }));
    onPaneLayoutChange.mockClear();
  ...
  test('永続化コールバック未注入でもドラッグは例外にならない', async () => {
    const { panel } = await createPanel();
    const splitter = widthSplitterEl(panel.root);
    stubRect(panesEl(panel.root), { width: 1200 });
    expect(() => {
  ...
  test('スプリッタにフォーカスがあるときは判定ショートカット（a 等）を発火させない', async () => {
    const { panel, onDecision } = await createPanel();
    widthSplitterEl(panel.root).dispatchEvent(
      new KeyboardEvent('keydown', { key: 'a', bubbles: true }),
    );
    expect(onDecision).not.toHaveBeenCalled();
    heightSplitterEl(panel.root).dispatchEvent(
      new KeyboardEvent('keydown', { key: 'a', bubbles: true }),
    );
    expect(onDecision).not.toHaveBeenCalled();
    panel.dispose();
