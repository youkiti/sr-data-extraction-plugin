...
describe('renderHomeView（reviewer 系ロールの縮退版 Home。§3・§7.2）', () => {
  ...
  test('フォルダアクセス付与済み: 検証への導線と再付与ボタンを出し、未付与用の付与ボタンは出さない', () => {
    const { ctx, callbacks } = makeCtx();
    const view = renderHomeView(reviewerState('reviewer_with_ai', { folderAccessGranted: true }), ctx);
    expect(view.querySelector('#home-grant-folder-access')).toBeNull();
    expect(view.querySelector('#home-go-verify')?.getAttribute('href')).toBe('#/verify');
    // issue #139: 付与はファイル単位のスナップショットのため、後から取り込まれた文献用に再付与できる
    const regrant = view.querySelector('#home-regrant-files') as HTMLButtonElement;
    expect(regrant.disabled).toBe(false);
    regrant.click();
    expect(callbacks.onGrantFolderAccess).toHaveBeenCalledTimes(1);
  });

  test('付与済みで確認中: 再付与ボタンを無効化し、確認中メッセージを出す', () => {
    const { ctx } = makeCtx();
    const view = renderHomeView(
      reviewerState('reviewer_with_ai', { folderAccessGranted: true, folderAccessChecking: true }),
      ctx,
    );
    const regrant = view.querySelector('#home-regrant-files') as HTMLButtonElement;
    expect(regrant.disabled).toBe(true);
    expect(view.querySelector('#home-folder-access-checking')?.textContent).toContain('確認しています');
  });

  test('付与済みで再付与が失敗: エラーを role="alert" で表示する', () => {
    const { ctx } = makeCtx();
    const view = renderHomeView(
      reviewerState('reviewer_with_ai', {
        folderAccessGranted: true,
        folderAccessError: '選択されていないファイルが 1 件あります',
      }),
      ctx,
    );
    const error = view.querySelector('#home-folder-access-error');
    expect(error?.textContent).toContain('選択されていないファイルが 1 件あります');
    expect(error?.getAttribute('role')).toBe('alert');
  });
