...
interface CapturedWrites {
  ...
  uploadCount: number;
  /** アップロードした Drive ファイルのファイル名（multipart メタデータから抽出。R セットの 8 ファイル検証用） */
  uploadedFileNames: string[];
  /** ensureChildFolder の POST 作成ボディから拾ったフォルダ名（rset_{timestamp}/ の命名検証用） */
  ...
  await page.route('https://www.googleapis.com/**', async (route) => {
    ...
    if (url.includes('/upload/drive/v3/files')) {
      ...
      if (nameMatch?.[1] !== undefined) {
        captured.uploadedFileNames.push(nameMatch[1]);
      }
      await route.fulfill({
    ...
    // createFolder（exports/ または rset_{timestamp}/ の新規作成）
    const body = route.request().postData() ?? '';
    const nameMatch = /"name":"([^"]+)"/.exec(body);
    if (nameMatch?.[1] !== undefined) {
      captured.createdFolderNames.push(nameMatch[1]);
    }
...
test('R セット選択 → サマリ/プレビュー → 生成で Drive フォルダ + 8 ファイル upload + ExportLog 追記 + UTF-8 案内文（issue #60）', async ({
  page,
}) => {
  const captured = await setupRoutes(page);
  await initApp(page, '#/export');
  ...
  await expect(page.locator('#export-format')).toBeVisible({ timeout: 15_000 });
  await page.locator('#export-format').locator('input[value=r_set]').check();

  // サマリ（ファイル数 / データ行数 / 未検証セル数 / export_issues 件数）+ ファイル一覧
  const summary = page.locator('#export-rset-summary');
  await expect(summary).toContainText('ファイル数');
  await expect(summary).toContainText('8');
  const fileList = page.locator('#export-rset-files');
  await expect(fileList).toContainText('tab1.csv');
  await expect(fileList).toContainText('export_manifest.json');

  // プレビューは ma.csv（本シナリオは ResultsData が空のためヘッダのみ = 0 行）
  await expect(page.locator('#export-rset-preview')).toBeVisible();

  const results = await new AxeBuilder({ page }).analyze();
  expect(results.violations).toEqual([]);

  // 生成（未検証 0 件 → 即実行。study_wide と同じ確認シナリオを流用）
  await page.locator('#export-generate').click();
  const resultCard = page.locator('#export-rset-result');
  await expect(resultCard).toBeVisible({ timeout: 15_000 });
  await expect(resultCard).toContainText(
    'フォルダに 8 ファイルを Drive に保存しました（ExportLog に記録済み）。',
  );

  // Drive: exports/ → rset_{YYYYMMDD-HHMMSS}/ の 2 段フォルダ作成 + 8 ファイル upload
  expect(captured.createdFolderNames).toContain('exports');
  expect(captured.createdFolderNames.some((name) => /^rset_\d{8}-\d{6}$/.test(name))).toBe(true);
  expect(captured.uploadCount).toBe(8);
  expect([...captured.uploadedFileNames].sort()).toEqual(
    [
      'tab1.csv',
      'tab1_status.csv',
      'ma.csv',
  ...
  // ExportLog: format='r_set' / study_count は tab1.csv 行数 / file_ref は保存先フォルダの webViewLink
  expect(captured.exportLogBodies).toHaveLength(1);
  const logRow = captured.exportLogBodies[0]?.values[0] as unknown as (string | number)[];
  expect(logRow[1]).toBe('r_set');
  expect(logRow[3]).toBe(1); // tab1.csv 行数（study-1 のみ確定 annotator あり。Jones 2021 は除外）
  expect(logRow[4]).toBe('https://drive/exports');

  // UTF-8（BOM なし）の初心者向け案内文（D-6）+ ローカル保存
  const note = page.locator('#export-rset-utf8-note');
  await expect(note).toContainText('UTF-8（BOM なし）');
  await expect(note).toContainText('readr::read_csv()');
  await expect(page.locator('#export-rset-result-link')).toHaveAttribute(
    'href',
    'https://drive/exports',
  );
