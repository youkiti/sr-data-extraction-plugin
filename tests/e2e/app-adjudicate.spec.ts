...
//   ⑤ axe
// を実弾（Sheets stub への書き込み URL・body の検証）で確認する。issue #63 の追加シナリオとして
// Evidence ハイライト + note 表示 / オフラインキュー退避 → 再送 / arm 並べ替えマッピングの既定対応 /
// 3 名以上のレビュアーのペア選択 → 裁定開始も本 spec で検証する
...
const REVIEWER_C = 'reviewer-c@example.com';
    ...
    win.chrome = {
      ...
      runtime: {
        ...
        sendMessage: async (msg: { type?: string }) => {
          ...
          if (msg?.type === 'auth:get-email') return { ok: true, email: userEmail };
          return { ok: true };
        },
      },
...
// study-3: 3 名（A・B は検証完了、C は未完了）→ pair=selectable（issue #63 の 3 人以上対応）
const STUDY_DATA_ROW_A3 = ['study-3', REVIEWER_A, 'human_with_ai', '1', '', 't0', '10', '60'];
const STUDY_DATA_ROW_B3 = ['study-3', REVIEWER_B, 'human_with_ai', '1', '', 't0', '10', '61'];
const STUDY_DATA_ROW_C3 = ['study-3', REVIEWER_C, 'human_with_ai', '1', '', 't0', '10', ''];
...
function decisionRow(
  ...
  return ['t0', annotator, studyId, fieldId, entityKey, annotator, 'human_with_ai', '1', action, value, note];
}
...
// study-3: A・B は study レベル 2 項目を判定済み（群構成は未確定 = arm セルは進捗分母に入らない）。
// C は StudyData 行のみで判定 0 件 → A×B の組だけがゲート達成になる（issue #63 のペア選択）
const STUDY3_DECISIONS = [
  decisionRow('f-mortality', '-', REVIEWER_A, 'accept', '10', '', 'study-3'),
  decisionRow('f-mortality', '-', REVIEWER_B, 'accept', '10', '', 'study-3'),
  decisionRow('f-age', '-', REVIEWER_A, 'accept', '60', '', 'study-3'),
  decisionRow('f-age', '-', REVIEWER_B, 'edit', '61', '', 'study-3'),
];
...
function documentsSlice(): Record<string, unknown> {
  return {
    records: [
      docRecord('doc-1', 'study-1', 'drive-1', 'smith2020.pdf'),
      docRecord('doc-2', 'study-2', 'drive-2', 'jones2021.pdf'),
      docRecord('doc-3', 'study-3', 'drive-3', 'brown2022.pdf'),
    ],
    studies: [studyRecord('study-1', 'Smith 2020'), studyRecord('study-2', 'Jones 2021'), studyRecord('study-3', 'Brown 2022')],
    extractedStudyIds: [],
...
test('study 一覧のゲート → 群構成の一致採用 → 一致セル一括採用 → 個別裁定（A 採用）まで実弾検証 + axe', async ({ page }) => {
  ...
  // ② 群構成: マッピングテーブル（issue #63）が既定の名称一致で対応済み + 「このまま採用」1 クリックで確定
  await expect(page.locator('#adjudicate-arm-card')).toContainText('一致しています');
  await expect(page.locator('#adjudicate-arm-map .adjudicate__arm-map-select')).toHaveValue('arm:1');
...
test('3 名以上のレビュアー: 一覧のペア選択でゲート達成の 2 名を選ぶと裁定を開始できる（issue #63）', async ({
  page,
}) => {
  await setupRoutes(page);
  await initApp(page);

  // study-3（3 名）はディムせずペア選択セレクトを出す（未選択のうちは開始ボタン無し）
  await expect(page.locator('#adjudicate-list')).toBeVisible({ timeout: 15_000 });
  const study3Row = page.locator('tr[data-study-id="study-3"]');
  await expect(study3Row).not.toHaveClass(/adjudicate__list-row--dimmed/);
  await expect(study3Row).toContainText('レビュアーが 3 名以上います');
  await expect(study3Row.locator('.adjudicate__open-button')).toHaveCount(0);

  const select = study3Row.locator('.adjudicate__pair-select');
  // プレースホルダ + C(3,2) = 3 組（email 昇順）
  await expect(select.locator('option')).toHaveCount(4);
  await expect(select.locator('option').nth(1)).toHaveText(`${REVIEWER_A} × ${REVIEWER_B}`);

  // C を含む組はゲート未達（C は判定 0 件）→ 開始ボタンが出ない
  await select.selectOption('1'); // reviewer-a × reviewer-c
  await expect(study3Row).toContainText('0/2');
  await expect(study3Row.locator('.adjudicate__open-button')).toHaveCount(0);

  // A×B は両者 2/2 → 開始できる
  await select.selectOption('0');
  await expect(study3Row).toContainText(`A（${REVIEWER_A}）: 2/2`);
  await study3Row.locator('.adjudicate__open-button').click();
  await expect(page).toHaveURL(/#\/adjudicate\?study=study-3$/);
  await expect(page.locator('#adjudicate-working')).toBeVisible();
  // 選択した 2 名（A×B）の現在値で突き合わせる: mortality 10/10 一致・age 60/61 不一致
  await expect(page.locator('#adjudicate-summary')).toContainText('一致 1 件 / 不一致 1 件');
