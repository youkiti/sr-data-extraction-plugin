...
test('起動時に batchGet で進捗カウントを読み、サマリ + ガードのディム解除に反映する', async ({ page }) => {
  ...
  // ヘッダ: プロジェクト名もプロジェクト選択ページへのリンク + 歯車は設定ページへのリンク
  ...
  await expect(page.locator('#app-open-options')).toHaveAttribute('href', '../options/options.html');
  ...
  await expect(page.locator('#app-context')).toHaveText('スキーマ 画面を表示しています');
