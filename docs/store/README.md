...
- **プライバシーポリシー URL**: `https://youkiti.github.io/sr-data-extraction-plugin/privacy-policy.html`（GitHub Pages。issue #214 で方針確定）。v0.1.0 提出時の暫定だった GitHub ファイル URL 方式（`blob/master/docs/store/privacy-policy.md`）から **2026-08-03 に差し替え済み**（ストアダッシュボードでの運用作業。[remaining-work-plan.md](../remaining-work-plan.md) の実機 / 運用テスト一覧を参照）。
