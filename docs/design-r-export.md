...
- `rob_tool`: スキーマに挿入されている RoB テンプレート種別（`rob2` / `robins_i` / `quadas3` / `quadas3_applicability` / `quips` = `robFields.ts` の `tool` 値）。**RoB テンプレート未挿入なら常に空文字**（ステータス語彙〔no_data 等〕は出さない = RoB という概念自体が存在しないことを表す）
- `rob_overall_judgement`: `rob:overall` ドメインの judgement 値（verified のときのみ）。`ma_status.csv` 側は対応する verification_status を持つ
- **estimate 単位オーバーライドとの解決（issue #109・2026-07-20 確定）**: 当該 outcome 行のインスタンスキーに対する overall のオーバーライド行（entity_key `rob:overall\|outcome:<key>` 等）が verified で存在すればそれを優先し、なければ study 単位の base（現行どおり）を複製する（requirements.md §3.3 の解決規則「オーバーライド優先」を ma.csv の join に適用したもの）
...
列: `study_id`, `study_label`, `tool`（`rob2` / `robins_i` / `quadas3` / `quadas3_applicability` / `quips`）, `domain_id`（entity_key の先頭セグメント `rob:<domain_id>` から）, `domain_label`, `sq_id`（signaling question 行のみ。[#61](https://github.com/youkiti/sr-data-extraction-plugin/issues/61) 実装後まで常に空）, `outcome_id`（result-level RoB。estimate 単位のオーバーライド行〔[#109](https://github.com/youkiti/sr-data-extraction-plugin/issues/109)〕は当該 outcome_result インスタンスキーを充填、base 行は空）, `entity_key`（原文・監査用）, `judgement`, `support`, `verification_status`, `schema_version`。
...
- **estimate 単位のオーバーライド行（issue #109・2026-07-20 確定）**: base 行（スキーマ駆動の全ドメイン。上記）に加え、S8 で宣言されたオーバーライド（entity_key `rob:<domain_id>\|outcome:<key>` 等）を**宣言分だけデータ駆動で**出力する（`outcome_id` に インスタンスキーを充填）。**保存行をそのまま出す方式**で、未評価 estimate の行は捏造しない: estimate × ドメインの評価値の解決（オーバーライドがあればそれ、なければ base）は R 側の join に委ね、解決規則は本節を正典とする（requirements.md §3.3 の「オーバーライド優先」）。ソートは study → domain（テンプレート順）→ `outcome_id`（base = 空が先頭、以降文字列昇順）
...
**注記（`unit` 列と `outcome_unit_reported` の区別。[#76](https://github.com/youkiti/sr-data-extraction-plugin/issues/76)、2026-07-13 追記）**: この表の `unit` 列は `SchemaField.unit`（スキーマ設計者が定義時に書く期待単位のメモ。多くの場合空欄）であり、連続アウトカムプリセットに追加した `outcome_unit_reported`（`field_name` の 1 つとして `data_dictionary.csv` に通常の行を持つ）とは別物。後者は「論文が実際にどの単位で報告したか」を study ごとに抽出した**値**であり、`ma.csv` の他の項目と同様に通常の値列としてそのまま出力される（ビルダー側の特別扱いは不要 — §4.2「値列: outcome_result レベルの全 `field_name` を列で横持ち」がそのまま適用される）。
...
- ~~`reported_unit`（[#76](https://github.com/youkiti/sr-data-extraction-plugin/issues/76)）~~ → **解決済み（2026-07-13）**: `outcome_unit_reported` を連続アウトカムプリセットへ追加。通常のスキーマ項目のため builder 側の変更は不要（§4.4 注記）
