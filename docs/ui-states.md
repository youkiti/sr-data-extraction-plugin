...
| | 同・プレビュー読み込み中 | `#tiab-loading`「tiab-review のシートを読み込んでいます…」+ プレビュー / 閉じるボタンを無効化。tiab-review の `References` / `Decisions` タブ（+ `Config` の `fulltext_ai_active_round`。`Config` 欠落は null 扱い）を Sheets API で直読みする（同一 Google アカウント・追加スコープ不要）。タブ欠落（tiab-review のシートでない）・アクセス不可・ヘッダ不正は `#tiab-error` に理由を表示 |
