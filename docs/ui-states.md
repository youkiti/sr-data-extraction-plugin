...
| | reviewer 系の縮退版 Home（v0.11・issue #139 でファイル単位付与に変更） | `reviewer_with_ai` / `reviewer_independent` / `adjudicator` は進捗カウントを一切読み込まず、プロジェクト名 + プロジェクト切替リンクのみを表示。ファイルアクセス未付与のうちは `#home-folder-access`（案内文〔全選択の操作説明込み〕 + 「プロジェクトファイルへのアクセスを付与（開いた一覧をすべて選択）」`#home-grant-folder-access`。確認中は `#home-folder-access-checking`「確認しています…」でボタン disabled、失敗は `#home-folder-access-error`〔`role="alert"`〕）。ボタンは Documents タブから必要ファイル ID（PDF = `drive_file_id` / 抽出テキスト = `text_ref`）を集めて Picker のファイル許可モード（`view=files` + `setFileIds`）を開き、全選択で 1 件ずつ drive.file を付与してもらう（共有フォルダの Picker 選択では配下ファイルへ読み取りが付与されないため。issue #62 実機で確定）。一部のみ選択は「選択されていないファイルが {n} 件あります…」を `#home-folder-access-error` に表示して弾く。付与対象 0 件（文献 0 件）は Picker を開かず付与済みへ。到達性確認（抽出テキスト 1 件の試し読み。抽出テキストが無い全スキャン PDF プロジェクトは先頭 PDF のメタデータ取得で代替）は伝播遅延に備えて最大 3 回・約 2 秒間隔でリトライ（リトライ対象は試し読みのみ。フラグ保存の失敗は到達性エラーと区別してそのまま表示）。付与フラグは `chrome.storage.local` にプロジェクト × アカウント単位で保存。付与済みは `#home-folder-access-granted`「プロジェクトファイルへのアクセスは付与済みです。」+ 「検証を開始する」`#home-go-verify`（`#/verify` リンク）+ **再付与ボタン** `#home-regrant-files`「アクセスを付与し直す（開いた一覧をすべて選択）」（付与後に owner が取り込んだ文献のぶんを追加付与する導線。確認中は disabled + `#home-folder-access-checking`、失敗は `#home-folder-access-error`）。非 owner の adjudicator も同じ付与ステップを踏み、未付与のうちは `#/verify` に加えて `#/adjudicate` もガードでブロックされる（issue #139） |
