...
| §3 `#/documents` | ✅ 実装済み（読み込み中 / 失敗 / 空 / 取り込み中の進捗行 / 一覧 + バッジ + study_label インライン編集。2026-07-02）。Drive Picker はホスト済みページ方式（[hosted/README.md](../hosted/README.md)）のため、**ページのデプロイと API キー設定が済むまで実機の Picker 起動は不可** |
