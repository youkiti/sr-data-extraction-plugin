...
| §2 Options | ✅ 状態 A / B とも実装済み（Gemini キーの trim 保存・空文字抑止・保存中無効化・完了/失敗表示）。既定モデル（保存 / 空文字で解除 / S5 への注入）も実装済み（2026-07-03） |
...
- **可視**: Gemini API キー入力（`type="password"` + `autocomplete="off"`）/ 保存ボタン / 既定モデル入力（下記「既定モデル」参照）/ 表示言語セレクタ（MVP は ja 固定・ディム）
...
| 状態 | 受入基準 |
|---|---|
| 未設定 | `#default-model-status` = `既定モデル: 未設定`（`options__status` と同じトンマナ）。入力 `#default-model` は空。`<datalist id="default-model-candidates">` に単価表 `MODEL_PRICING` のモデル ID を列挙（候補提示のみ・自由入力も可）。補足文「S5 スキーマ画面の初期値になります。単価表にないモデルはコスト概算が表示されません」 |
| 保存済み | `#default-model-status` = `既定モデル: 保存済み` + 保存値を `#default-model` へ表示（API キーと違いマスク不要） |
| 保存中 | `#save-default-model.disabled = true` |
| 保存完了 | `trim()` して保存 → `保存しました。`。**空文字は「未設定に戻す」**（`settings.defaultModel` を削除）→ `未設定に戻しました。`（API キーと違い空での解除を許す） |
