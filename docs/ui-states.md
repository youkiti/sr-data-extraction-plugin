...
| | 一覧（study ごと・ゲート付き） | `#adjudicate-list`（`<table>`。1 行 = 1 study）。human annotator が 1 名以下の行は `.adjudicate__list-row--dimmed` で「両者の検証完了待ちです」を表示し裁定を開始できない。ちょうど 2 名の行は自動でペア確定し、ゲート達成行のみ両者の完了状況「A（email）: n/m・B（email）: n/m」+ 「裁定を開始」ボタン（**内訳は完了状況の件数のみ。値・判定内容は一覧では見せない = 盲検の継続**）。3 名以上の行（issue #63 ④）はディムにせず、ペア選択セレクト `.adjudicate__pair-select`（プレースホルダ「2 名の組を選択…」+ 全 2 名組合せを「A × B」で列挙。email 昇順）を表示する。組を選択すると選択ペアの完了状況 `.adjudicate__pair-progress` を表示し、両者 100% のときのみ「裁定を開始」ボタンを出す。選択はセッション内のみで永続化しない。未選択（または選択が無効）のまま `?study=` で直接開こうとした場合は「裁定する 2 名のレビュアーを選択してください」の案内（`workingError`） |
