...
| 章 | 実装状況 |
|---|---|
| §1 Popup | ⚠️ 「現在のプロジェクト表示 + メインビューを開く」のみ実装。ログイン状態群・新規作成フォーム・既存 ID 検証は未実装 |
...
| §3 `#/documents` | ⚠️ 著作権注意書きの常時表示のみ実装。取り込み UI（Picker / 進捗 / 一覧）は未実装 |
| §3 その他ルート（protocol〜export） | ⚠️ プレースホルダ表示のみ（見出し + 未実装注記） |
...
| `#/documents` | 空 | 「Drive から PDF を取り込む」ボタン + 空状態説明 + 画面上部に「著作権フリー / 利用許諾済みの PDF のみ取り込んでください」の注意書き（常時表示。チェック UI は設けない） |
...
| | Methods 記載例カード | 形式選択カードの下に「論文 Methods 記載例」カード。言語タブ（English / 日本語）× ワークフロートグル（単一レビュアー / 二重独立。MVP では二重はディム）+ コピーボタン。コピー成功で「コピーしました」フィードバック。自動反映できない `{{ }}` はそのまま表示 + 「ご自身の情報に置き換えてください」注記（文案は [methods-boilerplate.md](methods-boilerplate.md)） |
