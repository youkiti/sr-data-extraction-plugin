...
| # | 操作 | 期待結果 | OK |
|---|---|---|---|
| 1 | owner: スプレッドシート + プロジェクトフォルダを reviewer アカウントへ共有（Google Drive の共有設定。閲覧以上） | reviewer アカウントでスプレッドシート・フォルダを開ける | [ ] |
| 2 | owner: Home のレビュアー管理カードで reviewer の email + role（`reviewer`）+ review_mode（`with_ai` または `independent`）を登録 | `Reviewers` タブに 1 行追記される | [ ] |
