...
| # | 操作 | 期待結果 | OK |
|---|---|---|---|
| 1 | `npm run build` → `chrome://extensions` でパッケージ化された `dist/` を読み込む | manifest の CSP（`script-src 'self' 'wasm-unsafe-eval'; object-src 'self'`）が Chrome に拒否されずロードできる | [ ] |
| 2 | CCITTFaxDecode 圧縮のスキャン PDF（実 PDF。無ければ `tests/e2e/pdf-ccitt.spec.ts` の合成 fixture を単体の PDF ファイルとして保存したもので代替可）を Drive から取り込み、`#/verify` または PDF ビューアで開く | 該当ページが白紙にならず、画像が描画される。DevTools の worker コンソール（`background/service-worker` ではなく PDF.js worker 側）に CSP 違反エラーや `Jbig2Error` / `#instantiateWasm` 系のエラーが出ない | [ ] |
| 3 | 同じ確認中に DevTools の Network タブで `wasm/jbig2.wasm` `standard_fonts/LiberationSans-Regular.ttf` `iccs/CGATS001Compat-v2-micro.icc` 等の取得を確認する | `web_accessible_resources` を追加していないが、拡張自身のページ（`chrome-extension://...`）からの取得のため 200 で返る | [ ] |
