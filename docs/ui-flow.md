...
| `1`〜`9` | **enum 項目の編集中のみ**（issue #254）: 許容値チップ列の N 番目を確定。チップ列のローカル `keydown` で拾うため、下記のグローバル判定キーとは別系統で衝突しない（「その他（自由入力）」には数字を割り当てない） | 同左 |
...
- 検証画面 2 ペインの最小幅（PDF ビューアは最小 600px 幅・判定項目枠は最小 360px 幅を想定。メインビューは最小幅 1280px）。**分割比率・右ペインの高さはユーザーが調整できる**（issue #193。スプリッタのドラッグ / キーボード操作。`settings.verifyPaneLayout` へ永続化し S6 / S8 で共有。詳細は [ui-states.md §3](ui-states.md) の「ペインサイズ調整（issue #193）」）。未設定時の既定は本行に記載の最小幅相当の従来比率・75vh 上限のまま
