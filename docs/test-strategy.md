...
| テキスト層 JSON | `tests/fixtures/pdf/*.json`（ページ別テキスト + span 座標。実 PDF から `tools/generate-pdf-fixture.ts` で生成） | **する** | anchoring / highlightMap の unit テスト（PDF 非依存で完結） |

- fixture 生成スクリプトは **anchoring 実装より先に**作る
...
2. テキスト層 fixture JSON のスキーマ（`generate-pdf-fixture.ts` 実装時）
