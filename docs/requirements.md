...
| input_mode | enum | ✓ | `pdf_native`（PDF を直接 LLM へ）/ `text_only`（※Q3）/ `text_with_page_images`（issue #176・高精度読み取りモード。text_only のテキストに加え、テキスト層のある文書の全ページ画像も併用添付する run 単位のオプトイン。既定は text_only のまま） |
...
- **高精度読み取りモード（issue #176・`input_mode = text_with_page_images`。2026-07-21）**: 既定の `text_only` は PDF.js の `getTextContent()` の items をコンテンツストリーム順に連結するだけで、列位置・セル境界・段組といった**表・図のレイアウト構造を保持しない**ため、多列表の抽出精度が落ちることがある（列の対応が崩れる・not_reported に倒れる等）。本モードは S6 / S7 の実行前画面で run 単位に選択でき、ON にすると `text_status = no_text_layer` でない（= テキスト層のある）全文書について、**本文テキストに加えて全ページのレンダリング画像も併用添付**する（`pdf_native` の「画像だけを送る」経路とは異なり、本文はそのまま残したうえで画像を「追加」する点が異なる）。プロンプトには「画像は表・図を正しく読むための補助であり、`quote` は必ず添付テキストから逐語で取ること」を明示し、quote アンカリングは従来どおり本文（`normalized text`）を基準に行う（画像は bbox 対応の対象外）。既定は `text_only` のままで、明示的に ON にしたときだけ画像トークンぶんコストが増える（既存利用者の挙動・コストは変えない）。選択中プロバイダが画像入力に非対応（`supportsImageInput = false`）なモデルでは選択自体を disabled にする（2026-07-21 時点で Gemini / OpenRouter / OpenAI 互換の 3 プロバイダは全て対応のため実際には常に選択可）。`no_text_layer` 文書を含む study では、その文書は従来どおり `pdf_native`（画像のみ）のまま扱う（本モードで「追加」の対象にはしない）。トークン予算（`planRun.estimateBatch`）は画像トークン（`APPROX_IMAGE_TOKENS_PER_PAGE` を流用）を対象文書ぶん加算し、超過は事前警告する。ベンチマークによる効果測定（表由来項目の正確度・not_reported 特異度・アンカリング成功率・実測コスト）は本 issue のスコープ外とし、別 PR で実施する（採否は測定結果で決める）
...
> **リスク（Q3 確定）**: LLM が PDF を直接読む場合（`input_mode = pdf_native`）、LLM の内部テキスト認識と PDF.js テキスト層が不一致になりうる（表の読み順、2 段組みの結合順）。**入力方式の既定選択は text_status で自動判定する**（`no_text_layer` → `pdf_native` = ページ画像添付、それ以外 → `text_only`）。born-digital PDF を「画像入力 vs テキスト入力の比較検証目的」で手動トグルする機能は実装しない（研究上の計測ニーズは `experiments/` のベンチに委ねる。ここまでは 2026-07-12 時点の確定を維持）。**ただし issue #176（2026-07-21）で例外を追加した**: text_only は表のレイアウト構造（列位置・セル境界）を保持しないという**プロダクション上の実害**（多列表の抽出精度低下）に対処するため、born-digital 文書にもページ画像を「本文に追加して」併用添付できる高精度読み取りモード（`input_mode = text_with_page_images`）を run 単位のオプトインとして S6 / S7 に追加した。これは「画像入力とテキスト入力のどちらが精度が高いかを比較する実験モード」ではなく「text_only の構造欠落を画像で補う実務上の救済モード」で、既定は無効（`text_only` のまま）・明示 ON 時のみコストが増える（§4.3 参照）。born-digital での抽出精度そのものの比較実験（研究目的）は引き続き `experiments/` のベンチに委ねる。
...
| Q3 | LLM への入力方式 | (a) PDF を直接送信（表・レイアウト理解に強い）(b) 抽出テキストのみ（アンカリング一致率に強い）。**確定（v0.12・2026-07-12）: 両対応を実装済み。入力方式の既定選択は text_status で自動判定する**（`no_text_layer` → `pdf_native` = ページ画像添付、それ以外 → `text_only`）。実装は planRun / executeRun / extractData v3・bbox 座標ハイライトまで含む（§5・[docs/handoff-scanned-pdf-native-highlight.md](handoff-scanned-pdf-native-highlight.md)）。born-digital を**比較実験目的**で画像入力へ回す手動トグルは実装しない（研究上の計測ニーズは `experiments/` のベンチに委ねる）。**2026-07-21 追記（issue #176）**: 上記の「比較実験トグルは作らない」方針とは別に、**text_only が表・図のレイアウト構造を保持しないという実務上の課題**への対処として、born-digital 文書にもページ画像を本文に追加併用できる高精度読み取りモード（`input_mode = text_with_page_images`。S6 / S7 の run 単位オプトイン・既定 OFF）を実装済み。効果測定（ベンチ）・採否判断は別 PR（issue #176 は実装 PR 完了後もクローズしない） |
