...
- **ステータス**: S1〜S10 実装済み・実機通し確認完了（2026-07-03）。残タスクは [docs/remaining-work-plan.md](remaining-work-plan.md) を参照。§10 の Q1〜Q10 はレビュー済み（Q8 の閾値のみベンチマーク設計時に最終確定）。**v0.10（study / document モデル）は全 3 フェーズ実装済み・実機通し確認済み（2026-07-09）**。実装済み機能と残タスクの全体像は [docs/status-and-roadmap-20260711.md](status-and-roadmap-20260711.md) を参照
- **関連ドキュメント**:
