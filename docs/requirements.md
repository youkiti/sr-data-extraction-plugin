...
| LLM（MVP 追加） | OpenRouter（OpenAI 互換 API。`OpenRouterProvider` を sr-query-builder から移植・2026-07-04。モデルセレクタの「その他（直接入力）」で任意モデル ID を指定可。カスタムモデルの管理 UI〔テスト保存・一覧〕は P1） |
| Node.js | ≥ 18 |
...
- **追記型・上書き禁止**: `Protocol` / `SchemaVersions` / `SchemaFields` / `ExtractionRuns` / `ArmStructures` / `Evidence` / `Decisions` は追記のみ。`StudyData` / `ResultsData` の各 annotator 行のみ「現在値」として上書き更新を許可し、変更履歴は `Decisions` への追記で監査する
...
**2 行プロトコル（v0.8）**: run 1 件につき、(1) 実行開始時に `status='running'` の行を **Evidence の追記より先に**追記し、(2) 実行完了時に確定 status（`done` / `partial_failure`）の行を同じ `run_id` でもう 1 行追記する（追記型の原則は維持）。これにより「`Evidence` の `run_id` は必ず `ExtractionRuns` で解決できる」不変条件が立ち、タブを閉じる・クラッシュ等で実行が中断しても running 行が残るため中断を検出できる。読み手の規約: run の完了 / 中断は「完了 status の行があるか」で判別し、抽出済み document の集計（S7 の既定選択・進捗カウントの pilot 実行数）には完了行のみを数える。中断 run の document は「未抽出」に戻るため、S7 の既定選択（未抽出の全件）がそのまま再開手段になる。
...
検証画面（S8）冒頭の「群構成の確定」（§3.3 の arm レベル、§4.2）の保存先。1 行 = 1 arm。確定・改訂のたびに**その document の全 arm 行を新しい version で追記**する（追記型 = 監査証跡を兼ねる）。document × annotator の最新 version が現在の確定内容で、**行が 1 件もない document は「arm 未確定」**（検証画面で arm / outcome_result タブをディム表示）。`arm_key` は `ResultsData.entity_key`（`arm:n` およびその複合キー）と join できる。

| 列 | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| document_id | string(uuid) | ✓ | |
| version | int | ✓ | document × annotator ごとに 1 から採番。確定・改訂のたびに全 arm 行を新 version で追記 |
...
| annotator / annotator_type | string / enum | ✓ | 確定操作を行った annotator（MVP では確定者本人の `human_with_ai`） |
...
| S3 | 文献取り込み | Drive Picker、テキスト層抽出とステータス表示。画面に PDF の外部送信先（LLM API のみ）の説明を常時表示 |
...
| S11 | Options / 設定 | API キー（Gemini / OpenRouter）、既定モデル（プルダウン + その他で直接入力。OpenRouter カスタムモデルの管理 UI は P1）、表示言語 |
...
- **実行の耐中断性**: 実行開始時に `ExtractionRuns` へ `status='running'` 行を先行追記してから `Evidence` を書き始め、完了時に確定 status の行を追記する（2 行プロトコル。§3.2 `ExtractionRuns`）。検証画面は `ExtractionRuns` に無い `run_id` の Evidence（プロトコル導入前の中断で生じた孤児）をエラーにせず未抽出扱いで除外し、S7 は中断 run の残り文献をバナーで案内する（既定選択に含まれるため、そのまま実行 = 再開）
...
| **P1** | 二重独立抽出 + 不一致解決画面（tiab-review「担当セット」の思想を移植 ※Q4）、tiab-review プロジェクト引き継ぎ、OpenRouter カスタムモデルの管理 UI（プロバイダ実装 + モデルセレクタは 2026-07-04 に MVP へ前倒し済み）、RoB テンプレートスキーマ（2026-07-07 に MVP へ前倒し済み ※v0.9）、UI 英語化 |
