...
| #214（GitHub Pages の公開ページ + ストア掲載への反映） | **運用作業（gh-pages デプロイ + ストアダッシュボード）** | **未実施**: ①`hosted/` の 4 ページ（index / help / privacy-policy / terms-of-service）+ `style.css` + `screenshots/` を `gh-pages` ルートへデプロイし（手順は [hosted/README.md](../hosted/README.md) の「公開ページ」節）、各 URL の表示・相互リンク・スクリーンショット表示・ja/en 並記の可読性を **PC 幅とスマホ幅**で確認 + `picker.html` が従来どおり動くこと ②ストアダッシュボードで**プライバシーポリシー URL** を `.../privacy-policy.html` へ差し替え、「ウェブサイト」欄へルート URL を設定し、審査への影響（再審査の要否・所要日数）を確認 ③拡張内の 3 導線（app ヘッダ「ヘルプ」/ popup フッタ / Options 末尾）からデプロイ後の実 URL が新規タブで開くこと（E2E は href の一致までしか担保しない） |
