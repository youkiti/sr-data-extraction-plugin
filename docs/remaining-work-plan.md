...
| [#127](https://github.com/youkiti/sr-data-extraction-plugin/issues/127) 各種 API への対応 | **オープン・段階実装中**。LLM プロバイダの拡充（[kouchou-ai-serverless](https://github.com/tokoroten/kouchou-ai-serverless) を参考）。**PR1（AnthropicProvider 本体）/ PR2（Options の接続方式として配線 = PR #209）/ PR3（Azure OpenAI。新規 provider クラスは作らず `OpenAICompatibleProvider` を認証方式だけ切り替えて流用）/ PR4（モデル一覧の自動取得。Anthropic / OpenRouter / OpenAI 互換 API のみ対応。Gemini / Azure OpenAI は対象外）はマージ済み**、Anthropic の実 API 疎通も確認済み〔下表 #127 行〕。**Azure OpenAI は実テナントでの疎通が未確認のまま残る**（本 PR はテナントを持たないため実施できず。docs/requirements.md §10 Q11 の③）。**残り**: Azure OpenAI の実機確認・PR5 reasoning effort の設定化 |
