...
- **作成日**: 2026-07-03（タスク A〜E）/ **更新**: 2026-07-12（v0.1.0 公開後のマイルストーン M1〜M4 を追加）・2026-07-18（#62 実機検証の §7.3 決着 = フォルダ付与不成立 → ファイル単位付与への設計変更〔#138/#139 = PR #140〕と進捗を反映）
- **対象読者**: 本リポジトリで作業する実装者
...
| issue | 内容 |
|---|---|
| [#62](https://github.com/youkiti/sr-data-extraction-plugin/issues/62) | 独立二重レビューの 2 アカウント実機通し確認（**進行中・前半決着**。設計成立条件 §7.3 は 2026-07-18 に実機で決着: 共有フォルダの Picker 選択では配下が読めず**不成立** → ファイル単位付与〔#138/#139 = PR #140〕へ設計変更し、**付与成功 + `#/verify` での PDF・抽出テキスト読出しまで実機確認済み**。残り = 検証〔with_ai / independent〕→ 裁定 → consensus エクスポートの通し） |
...
| [#141](https://github.com/youkiti/sr-data-extraction-plugin/issues/141) | ファイル単位付与の残課題（付与済み ID セットの永続化と差分再付与 / 削除済みファイル残存行の恒久ブロック解消 / `setFileIds` 件数上限 / hosted ページ整理・バンドルファイル方式の検討。#62 の通し成立後に着手判断） |
...
| #128〜#131（OAuth スコープ移行） | **実機（実 Google アカウント 2 つ + 実 Drive 共有）** | ①新同意画面が `userinfo.email` + `drive.file` の 2 権限のみで、認可応答の `scope` に `spreadsheets` が乗らない（実測）②旧アプリ作成シートが Picker 不要で開ける ③共有シート URL 貼り付け → 誘導 → Picker（setFileIds で対象のみ表示）→ picked → **即時再接続**（同一トークンへの Picker 付与の即時性。遅ければ再試行間隔 / 回数を増強）④picked ID 不一致・キャンセル・削除済みシートの最終文言 ⑤既存コラボレータ再入場 → ロールエラーの「Google で許可する」→ 復帰（issue #131）⑥ブラウザ再起動後のサイレント再認証（storage.local の email による login_hint）⑦Chrome プロファイルと別アカウント選択時の注意書きと記録 ⑧複数タブ同時操作で認可ウィンドウが多重に開かない ⑨link 共有のみ・Shared Drive 上のシート ⑩PDF Picker・フォルダ取り込み・レビュアー共有（`shareFileWithUser`）の無退行 ⑪ログアウト → 別アカウントログイン。**前提: リリース前に `hosted/picker.html` を gh-pages へデプロイ**（新拡張は nonce echo を必須検証。手順は [hosted/README.md](../hosted/README.md)）。`spreadsheets` の同意画面からの削除は #128 の削除判定基準（scope 実測 + 更新浸透目安 2 週間 + 実機確認完了）を満たしてから |
