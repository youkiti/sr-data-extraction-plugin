...
- **作成日**: 2026-07-03（タスク A〜E）/ **更新**: 2026-07-12（v0.1.0 公開後のマイルストーン M1〜M4 を追加）・2026-07-18（#62 実機検証の §7.3 決着 = フォルダ付与不成立 → ファイル単位付与への設計変更〔#138/#139 = PR #140〕と進捗を反映）・2026-07-19（#62 実機通し完了 = 検証 → 裁定 → consensus エクスポートまで全項目 OK。#62 クローズ・#63/#141 の残確認を更新。#142 Picker 許可導線の実機確認完了 → クローズを #68 行へ反映）・2026-07-29（**M1〜M4 表を GitHub issue の実状態と同期**。クローズ済み issue へ ✅ とクローズ日 / 対応 PR を付記し、未消化の作業だけが残るようにした）・2026-07-31（#127 PR5〔reasoning effort の設定化〕マージを反映。残るのは Azure OpenAI 実テナント疎通・Anthropic モデル一覧取得の実機確認・Anthropic 既定値 `low` の妥当性の実データベンチ確認の 3 点のみ）・2026-08-01（**#141 / #127 をクローズ**。実装は全てマージ済みで、残る実機 / 実 API 確認は「実機 / 実 API テストが必要な項目」の表で管理する運用〔実装のマージで issue は閉じる〕に統一）・2026-08-09（#249〔Google API 失敗の診断ログ・`ApiErrorLog` タブ新設〕を実機 / 実 API テスト表へ追加）・2026-08-09（#247〔Sheets 部分書き込みの検知〕の実装完了を反映し、「実機 / 実 API テストが必要な項目」へ行を追加）
- **対象読者**: 本リポジトリで作業する実装者
...
- **消化状況（2026-08-01 時点）**: **M1〜M4 の issue はすべてクローズ済み**（最後の [#141](https://github.com/youkiti/sr-data-extraction-plugin/issues/141) / [#127](https://github.com/youkiti/sr-data-extraction-plugin/issues/127) は 2026-08-01 にクローズ）。ただし**実装完了 ≠ 検証完了**で、クローズ済み issue でも実機 / 実 API 確認が残っているものがある（後述「実機 / 実 API テストが必要な項目」が正典。未消化の行が「やること」）。
...
| [#141](https://github.com/youkiti/sr-data-extraction-plugin/issues/141) | ファイル単位付与の残課題（**✅ 完了・クローズ済み（2026-08-01）**。課題 1・2・4 は 2026-07-19 に実装済み: 付与済み ID セット永続化 + 差分付与 + スキップ導線 = PR #150 / hosted ページ整理 + `page_version` ハンドシェイク + files モードの file_ids を ready 応答経由へ = PR #149・picker.html 2026-07-19a を gh-pages デプロイ済み。**残り = 実機のみ**: 差分付与の収束・スキップ・件数上限観察とバンドルファイル方式の要否判断〔下表 #141 行〕。件数上限が問題になれば別 issue で対応） |
...
| [#127](https://github.com/youkiti/sr-data-extraction-plugin/issues/127) 各種 API への対応 | **✅ 完了・クローズ済み（2026-08-01。実装は PR5 まで完了）**。LLM プロバイダの拡充（[kouchou-ai-serverless](https://github.com/tokoroten/kouchou-ai-serverless) を参考）。**PR1（AnthropicProvider 本体）/ PR2（Options の接続方式として配線 = PR #209）/ PR3（Azure OpenAI。新規 provider クラスは作らず `OpenAICompatibleProvider` を認証方式だけ切り替えて流用）/ PR4（モデル一覧の自動取得。Anthropic / OpenRouter / OpenAI 互換 API のみ対応。Gemini / Azure OpenAI は対象外）/ PR5（reasoning effort の設定化。Options `#default-reasoning-effort` + `ChatOptions.reasoningEffort`。未設定は各 provider の従来挙動を維持し、Anthropic ネイティブ = `output_config.effort` / OpenRouter・OpenAI 互換〔Azure OpenAI 含む〕= `reasoning_effort` へ写す。Gemini は未対応のまま据え置き）は全てマージ済み**、Anthropic の実 API 疎通も確認済み〔下表 #127 行〕。**残り（実装は無く実機 / 実 API 確認のみ）**: ① Azure OpenAI の実テナントでの疎通が未確認のまま残る（本 PR3 はテナントを持たないため実施できず。docs/requirements.md §10 Q11 の③）② PR4 の「モデル一覧を取得」（Anthropic `GET /v1/models`）が拡張オリジンからの `Origin` ヘッダー付きリクエストで同じブラウザアクセスヘッダ制約を受けるかの実機確認〔下表 #127 PR4 行〕③ PR5 の Anthropic 既定 effort `low` の妥当性の実データベンチ〔下表 #127 PR5 行〕。実機 / 実 API 確認で問題が出た場合は別 issue で対応 |
...
| #61 | 実 API（実 LLM） | **✅ 完了（2026-08-02。結論 = 出力トークン制約には当たらない。1 study 複数回コールへの分割は不要）**: 本番のプロンプト / `extractDataResponseSchema` / `planRun` / `GeminiProvider` をそのまま import した Node プローブ（`experiments/extraction-benchmark-real/src/robTokenProbe.ts`。gitignore 配下）で、実 gold ベンチの実 RCT 2 本 × 6 ケース = 14 コールを gemini-3.5-flash（prompt v9）で実行。**14/14 が `finishReason=STOP`**・format_error / rejected ゼロ。実測出力トークン: RoB2 SQ 単体 3,937〜4,364（24 項目 → 34 要素）/ ROBINS-I SQ 単体 4,389（36 → 50）/ QUADAS-3 単体 3,974〜4,561（37 → 51）/ QUIPS 単体 2,141（14 → 24）。**分割せず 92 項目を 1 コールに押し込む最悪ケースでも 16,427 トークンで打ち切りなし** = 既定上限は少なくとも 16.5k 超で、section 分割後の RoB バッチ（≤ 4.6k）には 3 倍以上の余裕がある。**限界**: ①ROBINS-I / QUADAS-3 / QUIPS は手持ちの実 PDF が RCT のみのため適用対象外の研究デザインに対する実測（プロンプトが「RCT なら全 not_reported」と指示するため値・quote が短くなる方向。ただし RoB2 実測の 116 トークン/要素を ROBINS-I の 50 要素に当てても ~5,800 トークンで余裕は大きい）②Gemini のみ（`executeRun` は `maxOutputTokens` を渡さず provider 既定に委ねるため、OpenRouter / OpenAI 互換 / Anthropic は未検証。とくに Anthropic は `DEFAULT_MAX_TOKENS = 16,000`〔`src/lib/llm/AnthropicProvider.ts`〕が効くので 1 コール最悪ケース級は打ち切られうる — 現行の section 分割が効く限り実害なし）。**副次的発見（#61 の判断は変えないが記録）**: `planRun` の出力トークン概算が RoB 系で ~2.2 倍過小（`ENTITY_INSTANCE_ESTIMATE.rob_domain = 1` だが judgement / support 行は 1 項目でドメイン数ぶんの要素を吐く）。また抽出 68 項目（概算 9,225・実測 11,031〜13,563）が `maxOutputTokensPerCall = 8,000` 超過の警告を出すが実際は完走しており、予算が保守的すぎる |
...
| #127 PR1 / PR2（Anthropic 対応） | 実 API（実 Anthropic キー）＝**主要な不確実性は解消済み（2026-07-30）** | **✅ 済み（Node からの実 API 疎通。ハーネスは `debug/anthropic-real-check.ts`・`debug/anthropic-cors-probe.ts`。debug/ は gitignore）**: ①Options 接続テストと同一ペイロード（`CONNECTION_TEST_SCHEMA` + `maxOutputTokens: 64`）が出荷 3 モデル（claude-opus-5 / claude-sonnet-5 / claude-haiku-4-5）すべてで `{"ok":true}` を返す = `max_tokens` の thinking headroom と haiku の effort deny list が実 API で妥当 ②**構造化出力のルート `type:'array'` は受理される**（`EXTRACT_DATA_RESPONSE_SCHEMA` / `DRAFT_SCHEMA_RESPONSE_SCHEMA` の実物で確認。`toAnthropicSchema` の `anyOf` 変換込み）→ 「公式が明示的に禁じていないため未決」だった §8-1 の懸念は解消 ③誤キーが `LlmProviderError(status=401)` へ写る ④**CORS は要 opt-in ヘッダと判明 → 修正済み**: `Origin` 付きリクエストは `anthropic-dangerous-direct-browser-access` が無いと 401（プリフライトも 400）、`true` を送ると 200 + `ACAO: *`。Chrome は拡張ページからの POST に Origin を付けるため必須で、host_permissions による CORS 免除では回避できない。**残り = 実機での形式確認のみ**（拡張の Options で「接続テスト」を押して成功トーストが出ること + S6 で 1 本パイロット抽出。Node では Origin が付かないため実機だけが最終確認になる） |
| #127 PR3（Azure OpenAI 対応） | 実 API（実 Azure テナント） | **未実施（テナントを持たないため本 PR では確認不可）**: ①実際のデプロイメント名 + `?api-version=...` クエリ文字列付き URL での疎通（`OpenAICompatibleProvider` を `authMode: 'azure_api_key'` で流用した実装が実 Azure レスポンス形式と噛み合うか）。②`api-key` ヘッダーでの認証が通ること。③拡張オリジン（`chrome-extension://`）からの `Origin` ヘッダー付きリクエストの挙動確認 — Anthropic 対応（#127 PR1/PR2）で `Origin` ヘッダーの有無だけで本文検査前に拒否される実例（`anthropic-dangerous-direct-browser-access` 要求）が見つかったため、Azure 側にも同種の未知の要件が無いか確認すること（事前にコードへ対策を先回りしない — 実機確認してから必要なら対応する） |
| #127 PR4（モデル一覧の自動取得） | 実機（拡張の実インストール）+ 実 API（実 Anthropic キー） | **未実施**: `GET https://api.anthropic.com/v1/models` は `POST /v1/messages` と全く同じ Origin ヘッダの制約（#127 PR1/PR2 で確認済みの `anthropic-dangerous-direct-browser-access` 要求）を受けるはずだが、これは jest（fetch はモック）では検証できない実 API 依存の前提。拡張の Options で Anthropic を選び「モデル一覧を取得」を押して成功トーストが出ることを実機で確認すること。OpenRouter（認証不要の公開エンドポイント）・OpenAI 互換 API（保存済み URL からの `/models` 導出）は一般的な REST 呼び出しのためリスクは相対的に低いが、実在のローカル / セルフホスト OpenAI 互換サーバでの疎通確認もあわせて行えるとよい |
| 単価表の未確認モデル（`pricing.ts`） | 実 API または公式料金ページ（**要ネットワーク到達**） | 2026-08-31 の一斉再確認で **Anthropic 3 モデルのみ公式ページに到達でき**、`claude-sonnet-5` を $3/$15 → $2/$10 へ修正した（導入価格が標準価格として恒久化されたため）。**未確認のまま残っているもの**: ①`gemini-2.5-pro` の単価（$1.25 / $10.00）②`gemini-2.0-flash` の単価（$0.10 / $0.40）と**モデル自体が提供終了になっている可能性** ③OpenRouter 2 モデル（`qwen/qwen3-235b-a22b-2507` の出力 $0.10 は第三者が取得したエンドポイント別ダンプの最安値 $0.55 すら下回っており過小表示の疑いが濃い。`deepseek/deepseek-v4-flash` も同様）。③は OpenRouter がマルチプロバイダで単価がレンジを持つため、確認時に「最安 / 中央値のどちらを採るか」の方針も決めること（既定ルーティングは最安固定ではない）。**推測で書き換えないこと** — 現状値のまま `pricing.ts` に要再確認コメントを置いてある |
| キャッシュヒット計測（`cachedTokensIn`） | 実 API（実 LLM・各 provider） | プロンプトキャッシュのヒット分をコスト計算から割り引く実装を入れた（`ChatResponse.cachedTokensIn` → `LLMApiLog.cached_tokens_in` / `estimateCostUsd`）が、**各 provider が実際にキャッシュ関連フィールドを返すかは実 API でしか確認できない**（jest は fetch をモック）。確認すること: ①Gemini が `usageMetadata.cachedContentTokenCount` を返し、`promptTokenCount` がその内数になっていること（tiab-review-plugin の実測では内数だったが本拡張のプロンプト形状では未確認）②Gemini の implicit caching の**実効閾値**（公式は「モデルにより 2,048〜4,096」だが tiab の実測では `gemini-3.1-flash-lite` で約 6,400 だった。公式ドキュメントへ到達できず裏が取れていない）③section 分割された run で実際にヒットが出ること（CLAUDE.md の「≈79% ヒット」の再現）④OpenRouter で `usage: { include: true }` を送ったときに `prompt_tokens_details.cached_tokens` が返ること ⑤Anthropic は `cache_control` を送っていないため常に 0 になること |
| 同一 study バッチの逐次化 | 実 API（実 LLM・`maxConcurrency ≥ 2`） | ワーカーへ配る単位をバッチ → study へ変えた（`groupBatchesByStudy`）。**キャッシュヒット率が実際に改善するかは実 API でしか測れない**: カスタム tier で同時実行数を 2 以上にし、section 分割される規模のスキーマで抽出を回して `LLMApiLog.cached_tokens_in` を変更前後で比較すること。あわせて跨 study のスループット（study/分）が落ちていないことも `experiments/throughput/aggregate.mjs` で確認する |
| #127 PR5（reasoning effort の設定化。Anthropic の既定値 `low` の妥当性） | 実 API（実データ抽出ベンチマーク） | **未実施**: `AnthropicProvider` の `DEFAULT_EFFORT = 'low'`（未設定時に送り続ける値）は、thinking が `max_tokens` を食い潰さないための**防御的な選択**であり、精度計測に基づく選定ではない（`AnthropicProvider.ts` の `DEFAULT_EFFORT` コメント参照）。PR5 で reasoning effort が利用者に設定可能になったことで、`low` を既定のまま据え置くか、`medium`/`high` の方が抽出精度で優れるかは未検証。`experiments/extraction-benchmark-real/` の実データ gold で Anthropic モデル（claude-opus-5 / claude-sonnet-5）を対象に `low`/`medium`/`high` を比較し、既定値を見直すべきか判断すること（本 PR ではベンチマークを実行しない） |
...
| #245（S1「最近のスプレッドシート」プルダウン） | 実 API（実 Drive・`drive.file` スコープ） | **未実施**: ①`files.list?q=mimeType='...spreadsheet'&orderBy=recency` が `drive.file` スコープで実際に返す集合（想定 = この拡張が作成したシート + Picker で許可したシートのみ）②**他人から共有された（Picker 許可済みの）スプレッドシートが候補に出ること**（本 issue の主目的。jest / E2E はどちらも `files.list` をモックするため実 API でしか確認できない）③候補に混ざる非プロジェクトシート（tiab-review のシート等）を選んだときに、遅延検証で既存の検証エラー文言が出ること |
| #247（Sheets 部分書き込みの検知） | 実 API（実 Sheets・大量行の append） | **未実施**: ① Evidence のような大量行（数十〜数百行）の `appendRows` で実 API が返す `updates.updatedRows` が実際に送信行数と一致すること（本 PR の検証ロジックは `updates.updatedRows` を最優先で信頼する設計のため、実 API がこのフィールドを常に正しく返すことが前提）。② `appendRows` へレスポンス検証（`SheetsPartialAppendError`）を追加したことで、既存の抽出フロー（S6/S7 のパイロット・一括抽出、Evidence の N study ごとのバッチフラッシュ）が壊れていないこと（誤検知で正常な run が `save_failed` に落ちないか）を実機で確認する |
| #214（GitHub Pages の公開ページ + ストア掲載への反映） | **運用作業（gh-pages デプロイ + ストアダッシュボード）** | **✅ 完了（2026-08-03）**: ①gh-pages ルートへデプロイ済み（`51da892` 初回 → `625aa61` 言語切替 i18n → `8880e91` 操作解説動画の埋め込み）。4 ページの表示・相互リンク・スクリーンショット表示・言語切替（切替ボタン / `?lang=en` / ページ遷移後の保持）を PC 幅・スマホ幅で確認 + `picker.html` の従来動作も確認 ②ストアダッシュボードでプライバシーポリシー URL を `https://youkiti.github.io/sr-data-extraction-plugin/privacy-policy.html` へ差し替え、「ウェブサイト」欄へ `https://youkiti.github.io/sr-data-extraction-plugin/` を設定 ③拡張内の 3 導線（app ヘッダ「ヘルプ」/ popup フッタ / Options 末尾）から実 URL が新規タブで開くことを実機確認。以下は当初の作業内容（記録として残す）: ①`hosted/` の 4 ページ（index / help / privacy-policy / terms-of-service）+ `style.css` + `lang.js` + `screenshots/` を `gh-pages` ルートへデプロイし（手順は [hosted/README.md](../hosted/README.md) の「公開ページ」節）、各 URL の表示・相互リンク・スクリーンショット表示・**言語切替**（切替ボタン / `?lang=en` / ページ遷移後の保持）を **PC 幅とスマホ幅**で確認 + `picker.html` が従来どおり動くこと ②ストアダッシュボードで**プライバシーポリシー URL** を `.../privacy-policy.html` へ差し替え、「ウェブサイト」欄へルート URL を設定し、審査への影響（再審査の要否・所要日数）を確認 ③拡張内の 3 導線（app ヘッダ「ヘルプ」/ popup フッタ / Options 末尾）からデプロイ後の実 URL が新規タブで開くこと（E2E は href の一致までしか担保しない） |
| #249（Google API 失敗の診断ログ。`ApiErrorLog` タブ新設） | **実機（実 API）確認が必要** | **未実施**: ①既存プロジェクト（`ApiErrorLog` タブ無し）で最初の失敗記録時にタブが自動作成されること（`Reviewers` 導入時と同じ後方互換パターンだが、jest は fetch モックのため実 Sheets での `addSheetTab` → `writeHeaderRow` → `append` の実挙動は未確認）②実際の失敗（例: 権限の無いファイル ID で Drive `alt=media` を叩いて 404、または一時的にネットワークを切って `pdf_load` を失敗させる）が `ApiErrorLog` へ `context` / `api` / `http_status` 付きで記録され、復旧後にローカルキューがフラッシュされること |
...
| 図表の高精度読み取りモード（issue #176） | 実 gold ベンチ（不眠 SR 10 論文・gemini-3.5-flash）で表由来項目正確度 +0.1pp・全体 +0.2pp と効果が誤差範囲、入力トークン +56%・コスト +14%。事前基準に従い実装（PR #178）ごと撤去。記録 = issue #176 コメント（2026-07-29）と非公開ベンチ REPORT-20260729 |
