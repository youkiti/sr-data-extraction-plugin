...
| #214（GitHub Pages の公開ページ + ストア掲載への反映） | **運用作業（gh-pages デプロイ + ストアダッシュボード）** | **①デプロイは実施済み（2026-08-02）**: `hosted/` の 4 ページ（index / help / privacy-policy / terms-of-service）+ `style.css` + `screenshots/`（4 枚）を `gh-pages` ルートへ push（手順は [hosted/README.md](../hosted/README.md) の「公開ページ」節）。`picker.html` と `.nojekyll` は無変更。デプロイ前にリンク検査（相互リンク・目次アンカー・画像パスの解決）を実施済み。**残り = ブラウザでの表示確認**: 各 URL の表示・スクリーンショット表示・ja/en 並記の可読性を **PC 幅とスマホ幅**で確認 + `picker.html` が従来どおり動くこと（開発コンテナからは `youkiti.github.io` へのアクセスがネットワークポリシーで 403 になるため未確認） ②**未実施**: ストアダッシュボードで**プライバシーポリシー URL** を `.../privacy-policy.html` へ差し替え、「ウェブサイト」欄へルート URL を設定し、審査への影響（再審査の要否・所要日数）を確認 ③拡張内の 3 導線（app ヘッダ「ヘルプ」/ popup フッタ / Options 末尾）からデプロイ後の実 URL が新規タブで開くこと（E2E は href の一致までしか担保しない） |
