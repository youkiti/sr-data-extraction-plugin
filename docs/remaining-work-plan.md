...
| #176（図表の高精度読み取りモード。実装 PR 完了・2026-07-21） | 実 API（実 LLM）+ 実機（実 PDF・拡張の実インストール） | (a) ベンチ実行（`experiments/extraction-benchmark-real` 主・`experiments/extraction-benchmark` 補。text_only vs `text_with_page_images` で表由来項目の正確度・全体正確度・not_reported 特異度・quote アンカリング成功率・実測トークン数/ コスト / 応答時間を比較。`supplement` role を持つ study を含める。コスト上限設定要）(b) UI からのモード選択と実 PDF での挙動（画像トークン超過時の応答遅延・429、画像非対応モデル選択時の 4xx ハンドリング、S6/S7 のコスト概算表示が実測と大きく乖離しないか）。**採否はベンチ結果で決める**: 改善が誤差なら本モードごと不採用とし、下記「不採用（記録）」へ移す |
