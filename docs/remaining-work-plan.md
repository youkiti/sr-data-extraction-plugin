...
| #109（QUADAS-3 flow 図 mermaid の遅延チャンク） | 実機（拡張の実インストール） | S8 で `quadas3_flow_diagram` セルの「図をプレビュー」を開き、mermaid の遅延チャンク（`dist/chunks/mermaid.js`。約 666 KiB）が `chrome-extension://` + MV3 CSP 環境で実際にロードされ SVG が描画されること（DevTools Network で `chunks/mermaid.js` の取得を確認）。E2E は http 静的配信で webpack publicPath 解決 + チャンク実ロードまで実弾で通したが、拡張実環境の動的 script 挿入 + CSP は http 配信では完全代替できないため実機で最終確認する（#95 層 1 の同梱 CMap ロード確認と同種の未担保領域）。#128 の release-build 実機確認とまとめて実施可能 |
