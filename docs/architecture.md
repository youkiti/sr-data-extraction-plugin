...
| PDF.js | `lib/pdf/*` をモジュールモック。**テキスト層 fixture**（`tests/fixtures/pdf/*.json`: ページ別テキスト + span 座標）を実 PDF から事前生成して使う。anchoring のテストは fixture のみで完結させ、canvas 描画は E2E でだけ実弾を使う |
...
| ランタイムバリデータ | zod | MIT |
| ID 生成 | uuid | MIT |
| 編集距離 | fastest-levenshtein（or 自前実装。実装時にバンドルサイズで判断） | MIT |
...
2. **PDF.js のバージョンと worker 同梱方式**（MV3 CSP との相性確認を含む）
...
4. **fuzzy マッチの実装**: fastest-levenshtein vs 自前 bitap
