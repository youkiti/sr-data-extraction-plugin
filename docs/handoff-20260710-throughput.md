...
> **2026-07-10 追記（Sheets 書き込み 429 対策・実装済み）**: 実機の並列スループット実測で「LLM 側ではなく **Google Sheets API の書き込みクォータ（60 回/分/ユーザー）**」に当たる不具合を発見。study ごとに `Evidence` を都度 `appendEvidence` していたため、並列化で短時間に集中すると 429 になっていた（詳細な原因分析・対策設計は [docs/handoff-20260710-sheets-write-batching.md](handoff-20260710-sheets-write-batching.md)）。対策 2 本立てを実装済み: **A. Evidence 書き込みのバッチ化**（`executeRun.ts`: `flushEveryNStudies`〔tier 連動。無料枠 5 / Tier 1=8 / Tier 2=12 / Tier 3・制限なし=15。custom は同時実行数から `clamp(round(maxConcurrency×2), 5, 15)` で逆算〕study ごと **または** `maxRowsPerFlush`〔既定 500。安全弁〕行ごと + 全 study 完了時にまとめて `appendEvidence`。フラッシュは直列化して二重フラッシュを防止し、失敗時は含まれる study を全部 `save_failed` として partial_failure に記録）+ **B. `googleFetch`（`src/lib/google/types.ts`）の 429/503 リトライ**（指数バックオフ + `Retry-After` 尊重 + `maxDelayMs` 上限。400/401/403/404 は従来どおり即 throw）。§6「触るべきファイル早見」の「Sheets 書き込みの間引き検討」は本対策で着手済み。jest 100% 維持・tsc/eslint/webpack green。**実機での効果確認（10〜20 study・同時実行数 4〜8 で 429 が出ない/自動回復すること）はこれから**。tier 連動・行キャップの詳細は [docs/handoff-20260710-sheets-write-batching.md](handoff-20260710-sheets-write-batching.md) §10。
...
| Sheets 書き込みの間引き（**実装済み・2026-07-10**） | `src/features/extraction/executeRun.ts`（バッチ化）/ `src/app/services/extractionService.ts`（`flushEveryNStudies` 注入）/ `src/lib/google/types.ts`（`googleFetch` の 429/503 リトライ）。詳細は [docs/handoff-20260710-sheets-write-batching.md](handoff-20260710-sheets-write-batching.md) |
