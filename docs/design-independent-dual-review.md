...
- **作成日**: 2026-07-11（v0.1 ドラフト・レビュー待ち）/ **ステータス**: v0.2・**実装済み**（2026-07-11。フェーズ 1〜3 = PR 相当のコミット d9d5c72 / 3816ccc / 61be220 / 6f050be 完了。jest 144 suites / 2042 tests green・カバレッジ 100%・E2E 64 本 green。**2 アカウントでの実機通し確認〔§7.3 のスパイク含む〕は 2026-07-19 に完了**〔issue #62。残る未実施は 3 アカウント目のペア選択確認のみ〕。§11 の「スコープ外」に挙げていた arm 並べ替えマッピング・3 人以上の reviewer 対応・一致率 / κ 統計は 2026-07-12〜07-13 に issue #63 / #66 で解消済み。設計からの逸脱・実装結果は §13 を参照）
...
4. **ファイルアクセス付与ステップ**（2026-07-18 改訂・issue #139。2026-07-19・issue #141 で差分付与へ改訂）: `drive.file` スコープでは他人が作成したファイルのバイナリを読めないため、Picker で本アプリに権限を付与する。旧設計の「プロジェクトフォルダを選択」は実機で不成立が確定した（§7.3 の検証結果）ため、Documents タブから必要ファイル ID（PDF = `drive_file_id` / 抽出テキスト = `text_ref`）を集めて Picker のファイル許可モード（`view=files` + `setFileIds`）に列挙し、reviewer にファイル単位で付与してもらう。**選択分は記録され、不足分のみ再提示される（収束型）**: 付与済みファイル ID（`granted` / `skipped`）を `chrome.storage.local` に永続化し、Picker には required からその集合を除いた不足分だけを列挙する。一部のみ選択しても付与漏れとして弾かない — 選択した分はその場で永続化し、残り不足分の件数を画面に表示するだけで、再度ボタンを押せば残りの不足分だけが Picker に列挙される。読めないファイル（Drive 上で削除済み等）はスキップして続行できる（該当文書は検証画面で個別に読み込みエラーになる。issue #141 課題 2: 削除済みファイルの残存 Documents 行が全選択ゲートを恒久ブロックする問題の回避）。加えて、付与済みで再入場した際は起動時に不足分の有無を静かに確認し（owner が後から取り込んだ文献の検知。issue #141 課題 1）、見つかれば同じ banner + スキップ導線を表示する
...
### 7.3 技術リスク（最優先の実機確認項目）→ 実機検証済み（2026-07-18・不成立が確定）
...
**実機検証の結果（2026-07-18・issue #62 の 2 アカウント通し・issue #139）**: 不成立が確定。reviewer アカウントで共有プロジェクトフォルダを Picker 選択しても配下ファイルの読み出しは HTTP 404 のままで、`extracted_texts` サブフォルダを直接選択（対象は直下ファイル）でも 404 だった。つまり**他人所有の共有フォルダでは、Picker のフォルダ選択は直下のファイルにすら読み取りを付与しない**（2026-07-09 の自分所有フォルダでの成功は一般化できなかった）。一方、共有スプレッドシート本体はファイル単位の Picker（issue #130 の `setFileIds` 方式）で付与に成功しているため、**フォールバック 2 を拡張した「必要ファイル ID を `setFileIds` で列挙して全選択」方式へ切り替えた**（§7.2 手順 4。抽出テキストも対象に含むので mime フィルタなし）。
...
- **v1 の簡略化（§6.4・§9・§11 で予告済みの範囲を実装でも維持 → 2026-07-13 時点で 5 点とも解消済み）**: ~~arm の並べ替えマッピング UI~~〔2026-07-13・issue #63 で解消。本節の「arm 並べ替えマッピング」項〕・~~一致率 / κ 統計~~〔2026-07-12・issue #66 で解消。一致率・κ・不一致一覧を実装〕・~~3 人以上の reviewer 対応~~〔2026-07-13・issue #63 でペア選択方式により解消。本節の「3 人以上のレビュアー対応」項〕。~~裁定画面の PDF ペインは表示 + ページ送り / ズーム / テキスト検索のみとし、Evidence ハイライトの再特定・`Decisions.note` の表示は実装しなかった~~ / ~~裁定の書き込み失敗時は `lib/storage/offlineQueue` への退避を実装せず、トースト表示のみに留めた~~ → **2026-07-12（issue #63）に解消**:
  - 裁定 PDF ペインの Evidence ハイライト: `app/views/adjudicatePdfPane.ts` が `features/verification/highlights.ts` の `buildDocumentHighlights`（検証画面 S8/S9 と同じ実装）を再利用し、表示中文書の Evidence を矩形化して表示する（色分けは検証画面と揃えず一律 'unverified' に簡略化）。セル一覧（`adjudicateView.ts`）の「根拠を表示」ボタン（AI の Evidence があるセルのみ）→ `focusAdjudicateEvidence` が該当文書へ切替え + ハイライトへジャンプする。AI の Evidence が無いセル（human_independent 由来・not_reported 等）はボタン自体を出さず従来どおり。study に Evidence が 1 件も無い（独立入力のみのペア）ときは PDF ペインに案内文を出す
  - `Decisions.note` の表示: `features/adjudication/cellMatch.ts` の `buildAdjudicationCells` が任意引数 `decisionsA` / `decisionsB`（各 annotator の Decisions 全量）を受け取り、セルごとに直近（decided_at 最新）の note を `AdjudicationCell.noteA` / `noteB` へ畳み込む。`adjudicateView.ts` が A / B の値の下に「A のメモ: …」の形で表示する
  - 裁定書き込みの offlineQueue 退避: `app/services/verificationService.ts` に `QueuedConsensusWrite`（`applyConsensusWrites` 1 回ぶんの引数を保持）を追加し、`QueuedDecisionWrite` との判別共用体 `QueuedWrite` として検証側と**同じ** `sharedDecisionQueue`（用途名 'decisions'）を共有する（`.decision` の有無による構造的判別。既存の `QueuedDecisionWrite` 構築箇所は無改修）。`persistConsensusWrite`（`persistDecisionWrite` の consensus 版）が即時保存 → 失敗時はキューへ退避 → 成功時は同キューに残る過去の退避分（判定・裁定どちらも）を再送する。楽観ロック（issue #64）は consensus 書き込みに導入しない（`expectedUpdatedAt` 無し）ため conflict 状態は返さない。`adjudicationService.ts` の `applyWrites` はキュー退避（'queued'）でも検証パネルと同じ「人間の判断は確定済み」の原則で `working.consensusDecisions` を楽観反映し、退避件数を `AdjudicateState.queuedWrites` へ反映してヘッダにバナー表示する
- **arm 並べ替えマッピング（§6.2・§11 の「位置対応固定」を 2026-07-13 に解消。issue #63）**: `features/adjudication/armMatch.ts` を「A の各群に対応する B の群」のマッピングベースへ改めた。既定マッピングは名称一致（trim）→ 位置対応 → 残り物同士の順で自動対応し（同名同順の従来ケースは従来どおり全一致、同名別順は自動で正しく対応する）、裁定者は群構成カードのセレクト（`.adjudicate__arm-map-select`）で手動変更できる（同じ B 群の二重選択は元の行の対応を自動解除して 1:1 を維持）。セル突き合わせは B 側の entity_key（`arm:n` / `outcome:...|arm:n`）を「B の armKey → 正準 armKey」辞書（`buildArmKeyRemap`。正準キー空間 = A の armKey + どの A にも対応しない B のみ群への新規採番キー）で書き換えてから行う（`remapArmEntityKey`）。これに伴い consensus 群構成ドラフトの armKey も「位置で `arm:n` を振り直す」から「A の armKey を引き継ぐ + B のみ群は新キー」へ変更し、consensus の `ArmStructures.arm_key` と consensus セルの entity_key が構造的に一致するようになった。**辞書は群構成確定時に `ArmStructures` の note へ `arm_mapping:{...}` 形式で直列化して永続化**し（`serializeArmKeyRemap` / `parseArmKeyRemapNote`）、再入場時に同じ突き合わせを復元する（note に辞書が無い旧データは既定マッピングにフォールバック）。マッピングの変更は consensus 群構成の確定前のみ可（確定後はセルの裁定が始まるため固定）
- **3 人以上のレビュアー対応（§11 のスコープ外を 2026-07-13 に解消。issue #63）**: N-way 比較ではなく**ペア選択方式**を採用した。`resolveAnnotatorPair` の 3 名以上は `ambiguous`（裁定不可）から `selectable` へ変更し、一覧読込時に全 2 名組合せ（C(n,2)）それぞれのゲートを事前計算（`AdjudicateStudyRow.pairOptions`）、裁定者が一覧のセレクトで 2 名を選ぶと選択ペアのゲート判定で裁定を開始できる（選択は `AdjudicateState.pairSelections` にセッション内のみ保持）。裁定・consensus 書き込み・undo は既存の A/B パイプラインを無変更で流用する。選ばれなかったレビュアーの値は当該裁定の比較・consensus に関与しない（必要なら別ペアを選び直して追加裁定できる。既裁定セルは裁定済み表示になる）。一致率 / κ レポート（issue #66）は従来どおり ready（ちょうど 2 名）の study のみを対象とし、3 名以上の study は統計から除外される（κ がペア統計であるため。N-way の担当セット分割は引き続き将来対応）
...
- ~~**未実施の確認事項**: 2 アカウント（owner + reviewer）を使った実機通し確認（§7.3 の drive.file スコープでの共有フォルダ配下読み出しスパイクを含む）は未実施。`docs/manual-testing.md` に確認手順を追記済みだが、実施は今後のタスク~~ → **2026-07-19 に完了**（issue #62）。`docs/manual-testing.md` §5-6-2 の #1〜#9 全項目が問題なしで記録済み: with_ai / independent 両モードの検証・arm 順序食い違いの名称一致自動対応・`arm_mapping` の再入場復元・consensus 行 + `Decisions` 追記・エクスポートの consensus 優先まで確認。**残る未実施は 3 アカウント目を使ったペア選択の確認（§5-6-3）のみ**
