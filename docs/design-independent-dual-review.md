...
- **作成日**: 2026-07-11（v0.1 ドラフト・レビュー待ち）
...
1. Google 側の共有: スプレッドシート + プロジェクトフォルダを reviewer のアカウントへ共有（**アプリからは行わず**、案内文言 + リンクで誘導。Drive の共有 API を叩くには追加スコープが要るため）
2. アプリ側の登録: email + role + review_mode を入力 → `Reviewers` へ追記
...
- Drive 共有操作のアプリ内実行（追加スコープ回避のため案内のみ）
