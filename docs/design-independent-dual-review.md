...
4. **ファイルアクセス付与ステップ**（2026-07-18 改訂・issue #139）: `drive.file` スコープでは他人が作成したファイルのバイナリを読めないため、Picker で本アプリに権限を付与する。旧設計の「プロジェクトフォルダを選択」は実機で不成立が確定した（§7.3 の検証結果）ため、Documents タブから必要ファイル ID（PDF = `drive_file_id` / 抽出テキスト = `text_ref`）を集めて Picker のファイル許可モード（`view=files` + `setFileIds`）に列挙し、reviewer が**全選択**してファイル単位で付与する。一部のみ選択は付与漏れとして弾き、読めるようになるまで検証入場をブロック
