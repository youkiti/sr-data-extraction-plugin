...
- **v1 の簡略化（§6.4・§9・§11 で予告済みの範囲を実装でも維持）**: arm の並べ替えマッピング UI・一致率 / κ 統計〔2026-07-11 に issue #66 で一致率・κ・不一致一覧は実装済みへ更新〕・3 人以上の reviewer 対応は未実装（設計どおりオーナー判断が必要なためスコープ外）。~~裁定画面の PDF ペインは表示 + ページ送り / ズーム / テキスト検索のみとし、Evidence ハイライトの再特定・`Decisions.note` の表示は実装しなかった~~ / ~~裁定の書き込み失敗時は `lib/storage/offlineQueue` への退避を実装せず、トースト表示のみに留めた~~ → **2026-07-12（issue #63）に解消**:
