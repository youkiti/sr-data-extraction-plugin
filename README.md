...
> 抽出項目（スキーマのフィールド）数に上限はありません。AI ドラフト時のプロンプト内にある「10〜40 個程度を提案する」という指示は、あくまで AI が一度に提案する項目数の目安であり、ユーザーが手動で追加できる項目数やスキーマ全体の項目数を制限するものではありません。

> **表・図の抽出精度について**: 既定の抽出モード（`text_only`）は PDF から抜き出したテキストを LLM へそのまま渡すため、**表の列位置・セル境界・段組といったレイアウト構造は保持されません**。多列の表（アウトカムを時点別・サブグループ別に細かく報告している表など）では、この理由で抽出精度が落ちることがあります。細かい表データまで正確に拾いたい場合は、一括抽出画面（パイロット / 本抽出）の「高精度読み取りモード」を run 単位で ON にしてください。テキストに加えてページのレンダリング画像も LLM へ送るため表・図の読み取り精度が上がりますが、**トークン消費量（コスト）が大幅に増えます**。既定は OFF で、明示的に ON にしない限り挙動・コストは変わりません。
