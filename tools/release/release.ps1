<#
.SYNOPSIS
  version バンプから提出用 zip までを一発で通す（.claude/skills/release-build 手順 0〜2 の実体）。
...
# 本番ビルドは .env の WEBAUTH_CLIENT_ID のみを読む（LOCAL_ は dev 優先用）。
# webpack も未設定ならエラーで止まるが、バンプ前にここで落としたほうが後始末が要らない
$envPath = Join-Path $repoRoot '.env'
if (-not (Test-Path $envPath) -or -not (Select-String -Path $envPath -Pattern '^WEBAUTH_CLIENT_ID=.' -Quiet)) {
...
# 既知の警告は PDF.js / mermaid / wasm のサイズ超過のみ。それ以外は見せる
